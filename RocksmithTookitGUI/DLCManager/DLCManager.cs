...
namespace RocksmithToolkitGUI.DLCManager
{
    public partial class DLCManager : UserControl
    {
        ...
        public void btn_PopulateDB_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(String.Format("One of the mandatory backend, folder is missing " + txt_TempPath.Text + ", " + Temp_Path_Import + ", " + pathDLC + ", " + old_Path_Import + ", " + broken_Path_Import + ", " + dupli_Path_Import, MESSAGEBOX_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Error));
                try
                {
                    DirectoryInfo di;
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show(ex.Message, MESSAGEBOX_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    MessageBox.Show("Can not open create folders ! " + txt_TempPath + "-" + Temp_Path_Import + "-" + pathDLC + "-" + old_Path_Import + "-" + broken_Path_Import + "-" + dupli_Path_Import);
                }
                //return;
            }
            ...
            try
            {
                ...
                {// 1. If hash already exists do not insert
                    OleDbDataAdapter dha = new OleDbDataAdapter("SELECT FullPath, Path, FileName, FileHash, FileSize, ImportDate, m.Import_Date FROM Import as i LEFT JOIN Main as m on m.File_Hash = i.FileHash OR m.Original_File_Hash = i.FileHash ORDER BY i.FileHash", cnn);
                    dha.Fill(dns, "Import");
                    dha.Dispose();
                    var tft = "\n Ignoring "; ;
                    ...
                    if (chbx_Additional_Manipualtions.GetItemChecked(30) && noOfRec > 0)
                        ...
                        for (m = 0; m < noOfRec; m++)
                        {
                            File.Delete(dns.Tables[0].Rows[i].ItemArray[0].ToString());
                            tft += "\n Deleting "+dns.Tables[0].Rows[m].ItemArray[0].ToString()+" as imported on " + dns.Tables[0].Rows[m].ItemArray[7].ToString();
                        }

                    rtxt_StatisticsOnReadDLCs.Text = tft+ noOfRec + "/"+m+" identical duplicates"+ "\n" + rtxt_StatisticsOnReadDLCs.Text;
                    ...
                    rtxt_StatisticsOnReadDLCs.Text = noOfRec + "/"+m+" New Songs 2 Import into MainDB" + "\n" + rtxt_StatisticsOnReadDLCs.Text;
                    ...
                    if (noOfRec > 0)
                    {
                        ...
                        for (i = 0; i <= noOfRec - 1; i++)
                        {
                            ...
                            if (errr)
                            {
                                ...
                                rtxt_StatisticsOnReadDLCs.Text = "removing illegal characters..." + "\n" + rtxt_StatisticsOnReadDLCs.Text;
                               ...
                               //MessageBox.Show("Chose: 1.Update\n2. Alternate\n3. Ignore", MESSAGEBOX_CAPTION, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);                           
                                ...
                                foreach (var file in files)
                                {
                                    ...
                                    if ((author.ToLower() == file.Author.ToLower() && author != "" && file.Author != "" && file.Author != "Custom Song Creator" && author != "Custom Song Creator") || (file.DLC_Name == info.Name))
                                    {
                                        if (file.DLC_Name.ToLower() == info.Name.ToLower())
                                            artist = AssesConflict(file, info, author, tkversion, DD, Bass, Guitar, Combo, Rhythm, Lead, Vocals, Tunings, b, norows, original_FileName, art_hash, audio_hash, audioPreview_hash, alist, blist);
                                        else
                                        {
                                            ...
                                            if (file.Version.ToInt32() < info.PackageVersion.ToInt32())
                                                ...
                                                else artist = AssesConflict(file, info, author, tkversion, DD, Bass, Guitar, Combo, Rhythm, Lead, Vocals, Tunings, b, norows, original_FileName, art_hash, audio_hash, audioPreview_hash, alist, blist);
                                            if (file.Version.ToInt32() == info.PackageVersion.ToInt32()) artist = AssesConflict(file, info, author, tkversion, DD, Bass, Guitar, Combo, Rhythm, Lead, Vocals, Tunings, b, norows, original_FileName, art_hash, audio_hash, audioPreview_hash, alist, blist);
                                ...
                                if (artist == "Update")
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            rtxt_StatisticsOnReadDLCs.Text = dupli_Path_Import + "\\" + filename + "\n" + rtxt_StatisticsOnReadDLCs.Text;
                                            if (!File.Exists(dupli_Path_Import + "\\" + filename))
                                                File.Move(old_Path_Import + "\\" + filename, dupli_Path_Import + "\\" + filename);
                                            else File.Delete(txt_RocksmithDLCPath.Text + "\\" + filename);
                                            Available_Duplicate = "Yes";
                                ...
                                if (artist == "Insert" || artist == "Update") //Common set of action for all
                                {
                                    ...
                                    var cmd = "UPDATE Main SET Available_Old=\""+ Available_Old+"\",";
        ...
        public void Translation_And_Correction(string dbp)
        {
            ...
            try
            {
                using (OleDbConnection cnn = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + DB_Path))
                {
                    ...
                    cmd2 = @"INSERT INTO Standardization (Artist, Album) SELECT DISTINCT Switch(S.Artist_Correction<>null,S.Artist_Correction,1=1,S.Artist) AS ArtistN, Switch(S.Album_Correction<>null,S.Album_Correction,1=1,S.Album) AS AlbumN
                    FROM Standardization AS S
                    WHERE(S.[Artist] <> S.[Artist_Correction] OR S.[Album] <> S.[Album_Correction]) AND((SELECT count(*) from Standardization AS SS WHERE SS.Artist = S.Artist_Correction and SS.Album = S.Album_Correction) = 0); ";
        ...
        public string AssesConflict(Files filed, DLCPackageData datas, string Fauthor, string tkversion, string DD, string Bass, string Guitar, string Combo, string Rhythm, string Lead, string Vocals, string tunnings, int i, int norows, string original_FileName, string art_hash, string audio_hash, string audioPreview_hash, List<string> alist, List<string> blist)
        {
            ...
            else
            {
                ...
                //frm.Show();
                DuplicatesManagement frm1 = new DuplicatesManagement(txt_DBFolder.Text, filed, datas, Fauthor, tkversion, DD, Bass, Guitar, Combo, Rhythm, Lead, Vocals, tunnings, i, norows, original_FileName, art_hash, audio_hash, audioPreview_hash, alist, blist);
                //frm1.Show();
                frm1.ShowDialog();
        ...
        private void btn_Standardization_Click(object sender, EventArgs e)
        {
            ...
            Standardization frm = new Standardization(DBb_Path);
            frm.Show();
        }
