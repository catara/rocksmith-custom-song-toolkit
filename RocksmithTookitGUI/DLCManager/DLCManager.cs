...
{
    public partial class DLCManagerWindow : UserControl
    {
        ...
        public DLCManagerWindow()
        {
            ...
            //RocksmithToolkitGUI.DLCManagerTemp.DataAccess da = new DLCManagerTemp.DataAccess();
            //da.Populate(ref DataGridView1, ref bsPeople, ref bsPositions, ref bsBadges);
            ...
            //bcapi breaking the project
            //txt_RocksmithDLCPath.Text = ConfigRepository.Instance()["general_rs2014path"];
            //-Add the new 
            //ctxt_TempPath.Text = ConfigRepository.Instance()["ManageTempFolder"] 
            loading = false;
            Set_DEBUG();
        }
        ...
        private void btn_TempPath_Click_1(object sender, EventArgs e)
        {
            using (var fbd = new VistaFolderBrowserDialog())
            {
                if (fbd.ShowDialog() != DialogResult.OK)
        ...
        private void btn_ReadFolder_Click_1(object sender, EventArgs e)
        {
            if ((Directory.Exists(txt_RocksmithDLCPath.Text)) && (Directory.Exists(txt_TempPath.Text)))
            {
                //- to remove comment when final
                ...
                string[] files = System.IO.Directory.GetFiles(pathDLC, "*_p.psarc");
                ...
                foreach (string s in files)
                {
                    ...
                    //- To remove usage of ee and laoding
                    Console.WriteLine("{0} : {1} : {2}", fi.Name, fi.Directory, loading);

                    //details end

                    rtxt_StatisticsOnReadDLCs.Text = "File " + i + " :" + s + "-------" + fi.GetHashCode() + "-----------" + fi.Length + fi.Attributes + "\n"+ rtxt_StatisticsOnReadDLCs.Text;
                    pB_ReadDLCs.Increment(i);
                    i++;

                    // UNPACK
                    var unpackedDir = Packer.Unpack(s, txt_TempPath.Text, true, true, false);
                }
            }
        }
...
rtxt_StatisticsOnReadDLCs.Text = "done" + "..." + rtxt_StatisticsOnReadDLCs.Text;
        ...
        private void cbx_FileName_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        {
            //MessageBox.Show(String.Format("One", MESSAGEBOX_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Error));
        }
        ...
        private void btn_DecompressAll_Click_1(object sender, EventArgs e)
        {
            //Show Intro database window
            frmMainForm frm = new frmMainForm();
            frm.Show();
...
//MessageBox.Show("-1");
        ...
        private void lbl_StatisticsOnReadDLCs_Click(object sender, EventArgs e)
        {
