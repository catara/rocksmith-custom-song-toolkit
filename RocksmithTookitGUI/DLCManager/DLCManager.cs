...
namespace RocksmithToolkitGUI.DLCManager
{
    public partial class DLCManager : UserControl
    {
            ...
            if (result1 == DialogResult.Yes)
                try
                {
                    ...
                    using (OleDbConnection cnn = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + DBPath))
                    {
                        ...
                        cmd = "DELETE FROM Import_AuditTrail WHERE Original_File_Hash IN (" + hash + ")";//txt_ID.Text;
        ...
        public void InsertIntoDBwValues(string ftable, string ffields, string fvalues)
        {
            var DB_Path = "";
            //MessageBox.Show("cleaninig");
            DB_Path = (chbx_DefaultDB.Checked == true ? MyAppWD : txt_DBFolder.Text) + "\\Files.accdb;";
            ...
            foreach (var file in files)
            {
                ...
                if (file.Is_Broken != "Yes" || (file.Is_Broken == "Yes" && !chbx_Additional_Manipulations.GetItemChecked(7))) //"8. Don't repack Broken songs")
                {
                    ...
                    if (ConfigRepository.Instance()["dlcm_AdditionalManipul58"] == "Yes") //59. @Pack try to get Track No again (&don't save)                        
                    {
                        ...
                        if (ConfigRepository.Instance()["dlcm_AdditionalManipul59"] == "Yes") //60.@Pack try to get Track No again(&save)
                        {
