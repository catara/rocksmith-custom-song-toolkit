...
namespace RocksmithToolkitGUI.DLCManager
{
    ...
    public partial class MainDB : Form
    {
        ...
        public string AppWD = AppDomain.CurrentDomain.BaseDirectory; //when repacking; old:+ "DLCManager\\external_tools"
        public string ExtAppWD = AppDomain.CurrentDomain.BaseDirectory + "DLCManager\\external_tools"; //when repacking; old:
        ...
        private void MainDB_Load(object sender, EventArgs e)
        {
            ...
            if (c("dlcm_AdditionalManipul102") == "Yes") chbx_Clean.Checked = true;
            else chbx_Clean.Checked = false;
            if (c("dlcm_AdditionalManipul128") == "Yes") chbx_SKU.Checked = true;
            else chbx_SKU.Checked = false;
            if (c("dlcm_AdditionalManipul129") == "Yes") chbx_Hide.Checked = true;
            else chbx_Hide.Checked = false;
            ...
            chbx_Last_Packed.Checked = ConfigRepository.Instance()["dlcm_AdditionalManipul64"] == "Yes" ? true : false;
            ...
            if (c("dlcm_AdditionalManipul127") == "Yes") chbx_FN.Checked = true;
            else chbx_FN.Checked = false;
            ...
            //Create Groups
            CreateGroups(); timestamp = UpdateLog(timestamp, "Created groups", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);


            //Add fields for sorting
            CreateFieldsDropdown(); timestamp = UpdateLog(timestamp, "Created sort Fields dropdown", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);


            //Create filters from Db or programatically (e.g. groups)
            cmb_Filter = GenerateFilterList(cmb_Filter, cnb, cnc); timestamp = UpdateLog(timestamp, "Generated Filter List", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);

            lbGroups = GenerateFilterListBox(lbGroups, cmb_Filter, cnb, cnc); timestamp = UpdateLog(timestamp, "Generated Filter List", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            ...
            if (c("dlcm_Debug") == "Yes")
            {
                ...
                chbx_ANDArtist.Checked = true;
                chbx_ANDAlbum.Checked = true;
                chbx_ANDFileName.Checked = true;
                chbx_ORArtist.Visible = false;
                chbx_ORAlbum.Visible = false;
                chbx_ORFileName.Visible = false;
            }
            ...
            if (txt_ID.Text != "" && txt_ID.Text != null)
            {
                ...
                {
                    timestamp = UpdateLog(timestamp, "Issues with folders location", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    ...
                    DialogResult result1 = MessageBox.Show("Do you want to update paths?\n as DB Repository has been moved from " + OLD_Path + "\n\n to " + c("dlcm_TempPath") + tmpp + "\n\n-" + cmd, MESSAGEBOX_CAPTION, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (result1 == DialogResult.Yes)
                    {
                        ...
                        timestamp = UpdateLog(timestamp, "Fixed Issues wirth folder locaTIONS IN THE LIB", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    ...
                    {
                        MessageBox.Show("Issues with Temp Folder and DB Reporsitory"); SaveOK = true;
                        return;
                    }
                }
            }
            ...
            //Update_Selected();
            ListSettings(); timestamp = UpdateLog(timestamp, "List Setting", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            timestamp = UpdateLog(timestamp, "Checking FTP and if \"output\" folder are existing", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            cbx_Format_SelectedValueChanged(null, null); timestamp = UpdateLog(timestamp, "folders Changed/Checked", false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            if (!SearchON)
            {
                ...
                chbx_ANDArtist.Visible = true;
                chbx_ANDAlbum.Visible = true;
                chbx_ANDFileName.Visible = true;
                chbx_ORArtist.Visible = true;
                chbx_ORAlbum.Visible = true;
                chbx_ORFileName.Visible = true;
            }
            else
                if (txt_Artist.Text != "" || txt_Title.Text != "" || txt_Album.Text != "" || txt_Description.Text != "" ||
                 txt_Live_Details.Text != "" || txt_OldPath.Text != "" || txt_Author.Text != "" || txt_ID.Text != "")
                    try
                    {
                        ...
                        var v = SearchCmd.Replace("  ", " ").Replace(n, " * ").Replace("WHERE AND", "WHERE ").Replace("WHERE  AND", "WHERE ").Replace("WHERE OR", "WHERE ").Replace("WHERE  OR", "WHERE ");
                        ...
                        if (no_R != 0)
                        {
                            ...
                            ErrorWindow frm10 = new ErrorWindow(ShowRetailFinds(cmd), "", "Found in Cache table", false, false, true, "", "", "", true);
                            frm10.Show();
                        }
                        ...
                        if (t == 0)
                        {
                            ...
                            SearchCmd = SearchCmd.Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("WHERE AND", "WHERE ").Replace("WHERE  AND", "WHERE ")
                                .Replace("WHERE OR", "WHERE ").Replace("WHERE  OR", "WHERE ").Replace("'", "\"");
                            oldSearchCmd = oldSearchCmd.Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("WHERE AND", "WHERE ").Replace("WHERE  AND", "WHERE ")
                                .Replace("WHERE OR", "WHERE ").Replace("WHERE  OR", "WHERE ").Replace("'", "\"");
                            ...
                            var g = "";
                            newcond = newcond.Replace("WHERE AND", "WHERE ").Replace("WHERE  AND", "WHERE ").Replace("WHERE OR", "WHERE ").Replace("WHERE  OR", "WHERE ");
                            var no_o = GetNoRecords(oldSearchCmd.Replace("WHERE AND", "WHERE ").Replace("WHERE  AND", "WHERE ").Replace("WHERE OR", "WHERE ").Replace("WHERE  OR", "WHERE "), cnb, cnc);
                            var no_n = GetNoRecords("SELECT " + c("dlcm_SearchFields") + " FROM Main u " + ("WHERE " + newcond).Replace("WHERE AND", "WHERE ")
                                .Replace("WHERE  AND", "WHERE ").Replace("WHERE OR", "WHERE ").Replace("WHERE  OR", "WHERE ") + " ORDER BY " + c("dlcm_OrderOfFields") + ";", cnb, cnc);
                            var no_a = GetNoRecords("Select * from Main", cnb, cnc);
                            g = oldcond + "\n\nSearch retuned no result, so\n1. (Rec: " + no_o + ") Resetting to old search criteria: " + condition + "\n\n" +
                                "2. (Rec: " + no_n + ") Search newly (new criteria only): " + newcond
                                 + "\n\n3. (Rec: " + no_a + ")Reset to Filter 0ALL";
                            showOptions4Research(g, newcond);
        ...
        private void showOptions4Research(string g, string newcond)
        {
            ErrorWindow frm2 = new ErrorWindow(g, "https://www.audiokinetic.com/download/", "Search returned no result!"
                , true, true, true, "Execute old Search", "Run only new Search terms", "Reset Search term", false);
            frm2.ShowDialog();
            ...
            if (frm2.B1) SearchCmd = oldSearchCmd;
            else if (frm2.B2) SearchCmd = ("SELECT " + c("dlcm_SearchFields") + " FROM Main u WHERE " + newcond + " ORDER BY " + c("dlcm_OrderOfFields") + ";")
                    .Replace("WHERE AND", "WHERE ").Replace("WHERE OR", "WHERE ");
            else cmb_Filter.Text = "0ALL";
        }
        ...
        public void ChangeRow()
        {
            ...
            catch (Exception ex) { tst = "Erro at killing audio... " + ex.Message; timestamp = UpdateLog(timestamp, tst, false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs); }
            ...
            if (databox.SelectedCells.Count > 0 && databox.Rows[databox.SelectedCells[0].RowIndex].Cells["ID"].ToString() != "")
            {
                ...
                try
                {
                    ...
                    if (SongRecord[0].ID != null)
                    {
                        ...
                        var scmd = "SELECT Arrangement_Name, RouteMask, Bonus, PlaythroughYBLink, Alternate, Favorite FROM Arrangements WHERE CDLC_ID=" + SongRecord[0].ID + GetArrOfficSQLTxt(arrangoff);
                        ...
                        if (norecs > 0) for (int j = 0; j < norecs; j++)
                            ...
                            {
                                var b = dnss.Tables[0].Rows[j][2].ToString();
                                var t = dnss.Tables[0].Rows[j][3].ToString();//b + "_" + a + "_" +
                                var a = dnss.Tables[0].Rows[j][4].ToString();
                                var f = dnss.Tables[0].Rows[j][5].ToString();
                                //if (t == "Lead") b=0;
                                ...
                                + (b == "True".ToLower() ? "_B_" : "") + (a == "True".ToLower() ? "_A_" : "") + (f == "Yes".ToLower() ? "_F_" : "")
                        ...
                        txt_Duplicate_Of.Text = SongRecord[0].Duplicate_Of.ToString();
                        ...
                        if (txt_BasedOn_GP.Text == "") { btn_BasedOn_GP.Enabled = false; btn_GetGP.Enabled = false; }
                        else { btn_BasedOn_GP.Enabled = true; btn_GetGP.Enabled = true; }
                        txt_ToDo_s.Text = SongRecord[0].ToDos;
                        txt_EoFPath.Text = SongRecord[0].EoFPath;
                        if (txt_EoFPath.Text == "") { btn_OpenEoF.Enabled = false; btn_SetEoFPath.Enabled = false; }
                        else { btn_OpenEoF.Enabled = true; btn_SetEoFPath.Enabled = true; }
                        txt_ToneDetails.Text = SongRecord[0].ToneDetails;
                        txt_CF_Author.Text = SongRecord[0].CF_Author;
                        txt_PackageDetails.Text = SongRecord[0].PackageDetails;
                        txt_BasedOn_Tabs.Text = SongRecord[0].BasedOn_Tabs; if (txt_BasedOn_Tabs.Text == "") btn_BasedOn_Tabs.Enabled = false; else btn_BasedOn_Tabs.Enabled = true;
                        txt_BasedOn_Youtube.Text = SongRecord[0].BasedOn_Youtube; if (txt_BasedOn_Youtube.Text == "") btn_BasedOn_Youtube.Enabled = false; else btn_BasedOn_Youtube.Enabled = true;
                        txt_BasedOn_CF.Text = SongRecord[0].BasedOn_CF; if (txt_BasedOn_CF.Text == "") btn_BasedOn_CF.Enabled = false; else btn_BasedOn_CF.Enabled = true;
                        txt_filePathonself.Text = SongRecord[0].BasedOn_CF; if (txt_filePathonself.Text == "") btn_filePathonself.Enabled = false; else btn_filePathonself.Enabled = true;
                        txt_SpotifySongID.Text = SongRecord[0].Spotify_Song_ID; if (txt_SpotifySongID.Text == "") btn_SetSpotifySongID.Enabled = false; else btn_SetSpotifySongID.Enabled = true;
                        txt_BasedOn_RB.Text = SongRecord[0].BasedOn_RB; if (txt_BasedOn_RB.Text == "") btn_BasedOn_RB.Enabled = false; else btn_BasedOn_RB.Enabled = true;

                        chbx_Found_on_CF.Checked = SongRecord[0].Found_on_CF == "Yes" ? true : false;
                        chbx_Found_on_CAPIRO.Checked = SongRecord[0].Uploaded_on_CAPIRO == "Yes" ? true : false;
                        chbx_Uploaded_on_CF.Checked = SongRecord[0].Uploaded_on_CF == "Yes" ? true : false;
                        ...
                        if (SongRecord[0].A440TunningFrecv == "440Hz") chbx_A440.Checked = true;
                        else if (!SongRecord[0].A440TunningFrecv.ToLower().Contains("non")) chbx_A440.CheckState = CheckState.Indeterminate;
                        else chbx_A440.Checked = false;
                        if (SongRecord[0].Is_Beta == "Yes") chbx_Beta.Checked = true;
                        else chbx_Beta.Checked = false;
                        if (SongRecord[0].Has_Alternate_Track == "Yes") chbx_AltTrack.Checked = true;
                        else chbx_AltTrack.Checked = false;
                        ...
                        //if (SongRecord[0].Has_Slide == "Yes") { chbx_Capo.Checked = true; chbx_Capo.Visible = true; }
                        //else { chbx_Capo.Checked = false; chbx_Capo.Visible = false; }
                        ...
                        if (SongRecord[0].Available_Old != "Yes") chbx_Avail_Old.Checked = File.Exists(c("dlcm_TempPath") + "\\0_old\\" + SongRecord[0].Original_FileName) ? true : false;

                        //chbx_Avail_Duplicate.Checked = false;
                        if (SongRecord[0].Available_Duplicate == "Yes")
                        {
                            chbx_Avail_Duplicate.Checked = true; btn_DuplicateFolder.Enabled = true; cmb_Duplicate.Enabled = true;

                            for (int k = cmb_Duplicate.Items.Count - 1; k >= 0; --k) cmb_Duplicate.Items.RemoveAt(k);//remove items
                            var cmdz = " FROM Pack_AuditTrail WHERE PackPath like \"%0_duplicate%\" AND CDLC_ID=" + txt_ID.Text + "";
                            var tcmd = "SELECT PackPath+\"\\\"+FileName, FileName " + cmdz;
                            DataSet dms = new DataSet(); dms = SelectFromDB("Pack_AuditTrail", tcmd, "", cnb, cnc);
                            var norecz = GetNoRec(dms, cnb, cnc);//dns.Tables[0].Rows.Count;
                            if (norecz > 0) for (int j = 0; j < norecz; j++)
                            {
                                cmb_Duplicate.Items.Add(dms.Tables[0].Rows[j][0].ToString());//add items
                                cmb_Duplicate.Text = dms.Tables[0].Rows[j][0].ToString();
                            }
                        }
                        else { chbx_Avail_Duplicate.Checked = false; btn_DuplicateFolder.Enabled = false; cmb_Duplicate.Enabled = false; cmb_Duplicate.Text = ""; }

                        //chbx_AlternateAvail
                        //chbx_AlternateAvail.Checked = true; chbx_AlternateAvail.Visible = true;
                        for (int k = cmb_Alternate.Items.Count - 1; k >= 0; --k) cmb_Alternate.Items.RemoveAt(k);//remove items
                        var cmd = "SELECT ID, Song_Title FROM Main WHERE Duplicate_Of =\"" + txt_ID.Text + "\" OR ID=" + (txt_Duplicate_Of.Text == "" ? 0 : txt_Duplicate_Of.Text) + " ORDER BY Duplicate_Of; ";
                        ...
                        {
                            cmb_Alternate.Items.Add(dns.Tables[0].Rows[j][0].ToString() + " - " + dns.Tables[0].Rows[j][1].ToString());//add items
                            cmb_Alternate.Text = dns.Tables[0].Rows[j][0].ToString() + " - " + dns.Tables[0].Rows[j][1].ToString();
                            chbx_AlternateAvail.Checked = true; chbx_AlternateAvail.Visible = true; cmb_Alternate.Visible = true;
                        }
                        else { chbx_AlternateAvail.Checked = false; chbx_AlternateAvail.Visible = false; cmb_Alternate.Text = ""; cmb_Alternate.Visible = false; }
                        ...
                        if (SongRecord[0].AI_TrackAvail == "Yes") chbx_AI_TrackAvail.Checked = true;
                        else chbx_AI_TrackAvail.Checked = false;
                        ...
                        if (SongRecord[0].Has_Other_Officials == "Yes")
                        {
                            ...
                            var cmdd = "SELECT DISTINCT Platform FROM Pack_AuditTrail WHERE Official=\"Yes\" AND DLC_ID=" + SongRecord[0].ID + " AND PackPath not like \"%0_old%\"; ";
                            DataSet dds = new DataSet(); dds = SelectFromDB("Main", cmdd, "", cnb, cnc);
                            var norecd = GetNoRec(dds, cnb, cnc);//dns.Tables[0].Rows.Count;
                            if (norecd > 0) for (int j = 0; j < norecd; j++)
                            {
                                chbx_Format_Originals.Items.Add(dds.Tables[0].Rows[j][0].ToString());//add items
                                chbx_Format_Originals.Text = dds.Tables[0].Rows[j][0].ToString();
                            }
                        }
                        ...
                        txt_OldPath.Text = File.Exists(c("dlcm_TempPath") + "\\0_old\\" + SongRecord[0].Original_FileName) ? SongRecord[0].Original_FileName : "orignal imported file missing: " + SongRecord[0].Original_FileName;
                        ...
                        var sel = "SELECT Arrangement_Name, Start_Time, Bonus, Part, Has_Sections, MaxDifficulty, ToneBase, ToneA, ToneB, ToneC, ToneD, Tunning, TuningPitch, ConversionDateTime," +
                            " Rating, ScrollSpeed, Comments, XMLFilePath, Favorite, Alternate, Slide, CapoFret, A440TunningFrecv, ID FROM Arrangements WHERE CDLC_ID=" + txt_ID.Text + GetArrOfficSQLTxt(arrangoff) + " ORDER BY ID DESC;";
                        ...
                        {
                            var tzt = "";
                            var l = 0; var c = 0; var b = 0; var r = 0;
                            chbx_LeaF1.Checked = false; chbx_LeaF2.Checked = false; chbx_LeaF3.Checked = false; chbx_LeaF1.ForeColor = System.Drawing.Color.Green; chbx_LeaF2.ForeColor = System.Drawing.Color.Green; chbx_LeaF3.ForeColor = System.Drawing.Color.Green;
                            chbx_ComF1.Checked = false; chbx_ComF2.Checked = false; chbx_ComF3.Checked = false; chbx_ComF1.ForeColor = System.Drawing.Color.Green; chbx_ComF2.ForeColor = System.Drawing.Color.Green; chbx_ComF3.ForeColor = System.Drawing.Color.Green;
                            chbx_RhyF1.Checked = false; chbx_RhyF2.Checked = false; chbx_RhyF3.Checked = false; chbx_RhyF1.ForeColor = System.Drawing.Color.Green; chbx_RhyF2.ForeColor = System.Drawing.Color.Green; chbx_RhyF3.ForeColor = System.Drawing.Color.Green;
                            chbx_BasF1.Checked = false; chbx_BasF2.Checked = false; chbx_BasF3.Checked = false; chbx_BasF1.ForeColor = System.Drawing.Color.Green; chbx_BasF2.ForeColor = System.Drawing.Color.Green; chbx_BasF3.ForeColor = System.Drawing.Color.Green;

                            chbx_LeaF1.Visible = false; chbx_LeaF2.Visible = false; chbx_LeaF3.Visible = false;
                            chbx_ComF1.Visible = false; chbx_ComF2.Visible = false; chbx_ComF3.Visible = false;
                            chbx_RhyF1.Visible = false; chbx_RhyF2.Visible = false; chbx_RhyF3.Visible = false;
                            chbx_BasF1.Visible = false; chbx_BasF2.Visible = false; chbx_BasF3.Visible = false;
                            for (var j = 0; j <= rec - 1; j++)
                            {
                                ...
                                var fav = ", " + (ddr.Tables[0].Rows[j].ItemArray[18].ToString() == "Yes" ? "F" : "");
                                var alt = ", " + (ddr.Tables[0].Rows[j].ItemArray[19].ToString() == "True" ? "A" : "");
                                var sld = ", " + (ddr.Tables[0].Rows[j].ItemArray[20].ToString() == "Yes" ? "S" : "");
                                var capfr = ", " + (ddr.Tables[0].Rows[j].ItemArray[21].ToString() != "0" ? "CapoFret" + ddr.Tables[0].Rows[j].ItemArray[21].ToString() : "");
                                var a440 = ", " + (ddr.Tables[0].Rows[j].ItemArray[22].ToString() != "440" ? ddr.Tables[0].Rows[j].ItemArray[22].ToString() : "440");
                                var iud = ddr.Tables[0].Rows[j].ItemArray[23].ToString();

                                // Set up the ToolTip text for the Button and Checkbox.
                                var txtt = Start_Time + Bonus + fav + alt + a440 + sld + capfr + Part + HasSections + MaxDifficulty + Tones + Tunings + TuningPitch + lastConversion + Rating + ScroolSpeed + Comment;
                                txtt = txtt.Replace(", , ", ", ").Replace(", , ", ", ").Replace(", , ", ", ").Replace(", , ", ", ");
                                if (Arrangement_Name == "0" || Arrangement_Name == "Lead")
                                {
                                    toolTip1.SetToolTip(chbx_Lead, toolTip1.GetToolTip(chbx_Lead) + txtt); l++;
                                    if (l == 1) { chbx_LeaF1.Visible = true; toolTip1.SetToolTip(chbx_LeaF1, "Mark Lead 1 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_LeaF1.Checked = true; chbx_LeaF1.Text = iud; chbx_LeaF1.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_LeaF1, "Lead 1 is Marked as favorite(" + Comment + "):" + iud); } }
                                    else if (l == 2) { chbx_LeaF2.Visible = true; toolTip1.SetToolTip(chbx_LeaF2, "Mark Lead 2 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_LeaF2.Checked = true; chbx_LeaF2.Text = iud; chbx_LeaF2.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_LeaF2, "Lead 2 is Marked as favorite(" + Comment + "):" + iud); } }
                                    else if (l == 3) { chbx_LeaF3.Visible = true; toolTip1.SetToolTip(chbx_LeaF3, "Mark Lead 3 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_LeaF3.Checked = true; chbx_LeaF3.Text = iud; chbx_LeaF3.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_LeaF3, "Lead 3 is Marked as favorite(" + Comment + "):" + iud); } }
                                }
                                else if (Arrangement_Name == "1" || Arrangement_Name == "Rhythm")
                                {
                                    toolTip1.SetToolTip(chbx_Rhythm, toolTip1.GetToolTip(chbx_Rhythm) + txtt); r++;
                                    if (r == 1) { chbx_RhyF1.Visible = true; toolTip1.SetToolTip(chbx_RhyF1, "Mark Rhythm 1 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_RhyF1.Checked = true; chbx_RhyF1.Text = iud; chbx_RhyF1.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_RhyF1, "Rhythm 1 is Marked as favorite:" + iud); } }
                                    else if (r == 2) { chbx_RhyF2.Visible = true; toolTip1.SetToolTip(chbx_RhyF2, "Mark Rhythm 2 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_RhyF2.Checked = true; chbx_RhyF2.Text = iud; chbx_RhyF2.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_RhyF2, "Rhythm 2 is Marked as favorite:" + iud); } }
                                    else if (r == 3) { chbx_RhyF3.Visible = true; toolTip1.SetToolTip(chbx_RhyF3, "Mark Rhythm 3 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_RhyF3.Checked = true; chbx_RhyF3.Text = iud; chbx_RhyF3.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_RhyF3, "Rhythm 3 is Marked as favorite:" + iud); } }
                                }
                                else if (Arrangement_Name == "2" || Arrangement_Name == "Combo")
                                {
                                    toolTip1.SetToolTip(chbx_Combo, toolTip1.GetToolTip(chbx_Combo) + txtt); c++;
                                    if (c == 1) { chbx_ComF1.Visible = true; toolTip1.SetToolTip(chbx_ComF1, "Mark Combo 1 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_ComF1.Checked = true; chbx_ComF1.Text = iud; chbx_ComF1.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_ComF1, "Combo 1 is Marked as favorite(" + Comment + "):" + iud); } }
                                    else if (c == 2) { chbx_ComF2.Visible = true; toolTip1.SetToolTip(chbx_ComF2, "Mark Combo 2 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_ComF2.Checked = true; chbx_ComF2.Text = iud; chbx_ComF2.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_ComF2, "Combo 2 is Marked as favorite(" + Comment + "):" + iud); } }
                                    else if (c == 3) { chbx_ComF3.Visible = true; toolTip1.SetToolTip(chbx_ComF3, "Mark Combo 3 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_ComF3.Checked = true; chbx_ComF3.Text = iud; chbx_ComF3.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_ComF1, "Combo 3 is Marked as favorite(" + Comment + "):" + iud); } }
                                }
                                else if (Arrangement_Name == "3" || Arrangement_Name == "Bass")
                                {
                                    toolTip1.SetToolTip(chbx_Bass, toolTip1.GetToolTip(chbx_Bass) + txtt); b++;
                                    if (b == 1) { chbx_BasF1.Visible = true; toolTip1.SetToolTip(chbx_BasF1, "Mark Bass 1 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_BasF1.Checked = true; chbx_BasF1.Text = iud; chbx_BasF1.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_BasF1, "Bass 1 is Marked as favorite(" + Comment + "):" + iud); } }
                                    else if (b == 2) { chbx_BasF2.Visible = true; toolTip1.SetToolTip(chbx_BasF2, "Mark Bass 2 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_BasF2.Checked = true; chbx_BasF2.Text = iud; chbx_BasF2.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_BasF2, "Bass 2 is Marked as favorite(" + Comment + "):" + iud); } }
                                    else if (b == 3) { chbx_BasF3.Visible = true; toolTip1.SetToolTip(chbx_BasF3, "Mark Bass 3 as favorite(" + Comment + "):" + iud); if (fav == ", F") { chbx_BasF3.Checked = true; chbx_BasF3.Text = iud; chbx_BasF3.ForeColor = System.Drawing.Color.Red; toolTip1.SetToolTip(chbx_BasF3, "Bass 3 is Marked as favorite(" + Comment + "):" + iud); } }
                                }
                                ...
                                toolTip1.SetToolTip(txt_Tuning, toolTip1.GetToolTip(txt_Tuning) + ", " + instr + ": " + (bon == "" ? "" : bon + "-")
                                    + (fav == ", " ? "" : "F-") + (bon == ", " ? "" : "B-") + (alt == ", " ? "" : "A-") + (sld == ", " ? "" : "S-")
                                    + (capfr == ", " ? "" : "C" + capfr + "-") + (a440 == ", " ? "" : a440 + "-") + Tunings);
                ...
                catch (Exception ex) { tst = "Error at change row... " + ex.Message; timestamp = UpdateLog(timestamp, tst, false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs); }
                Panel1.Visible = true;
            }
        ...
        private void SetAttributes(MainDBfields[] SongRecord)
        {
            //Attributes
            //Location: 4, 54
            var x = 1;
            ...
            if (SongRecord[0].Has_Acoustic == "Yes") { chbx_A_HasAcoustic.Location = new Point(x, y); x += chbx_A_HasAcoustic.Size.Width; chbx_A_HasAcoustic.Checked = true; chbx_A_HasAcoustic.Visible = true; }
            else { chbx_A_HasAcoustic.Checked = false; chbx_A_HasAcoustic.Visible = false; }
            if (SongRecord[0].Is_Censored == "Yes") { chbx_A_IsCensored.Location = new Point(x, y); x += chbx_A_IsCensored.Size.Width; chbx_A_IsCensored.Checked = true; chbx_A_IsCensored.Visible = true; }
            else { chbx_A_IsCensored.Checked = false; chbx_A_IsCensored.Visible = false; }
            ...
            if (SongRecord[0].Has_Slide == "Yes") { chbx_A_IsSlide.Location = new Point(x, y); x += chbx_A_IsSlide.Size.Width; chbx_A_IsSlide.Checked = true; chbx_A_IsSlide.Visible = true; }
            else { chbx_A_IsSlide.Checked = false; chbx_A_IsSlide.Visible = false; }
            if (SongRecord[0].AI_TrackAvail == "Yes") { chbx_A_HasAITracksAvail.Location = new Point(x, y); x += chbx_A_HasAITracksAvail.Size.Width; chbx_A_HasAITracksAvail.Checked = true; chbx_A_ProductionReady.Visible = true; }
            else { chbx_A_HasAITracksAvail.Checked = false; chbx_A_HasAITracksAvail.Visible = false; }
            if (SongRecord[0].ProductionReady == "No") { chbx_A_ProductionReady.Location = new Point(x, y); x += chbx_A_ProductionReady.Size.Width; chbx_A_ProductionReady.Checked = true; chbx_A_ProductionReady.Visible = true; }
            else { chbx_A_ProductionReady.Checked = false; chbx_A_ProductionReady.Visible = false; }
            //if (SongRecord[0].AI_TrackAvail == "Yes") { chbx_AI_TrackAvail.Location = new Point(x, y); x += chbx_AI_TrackAvail.Size.Width; chbx_AI_TrackAvail.Checked = true; chbx_AI_TrackAvail.Visible = true; }
            //else { chbx_AI_TrackAvail.Checked = false; chbx_AI_TrackAvail.Visible = false; }
        }
        ...
        public void ListTracks(string Duplicate_Of, string ID, string Song_Lenght)
        {
            ...
            scmd = "SELECT ID, XMLFileName, Start_Time, RouteMask, Bonus, ArrangementType, CDLC_ID" +
                ", Alternate, Favorite, Slide, A440TunningFrecv, CapoFret FROM Arrangements WHERE CDLC_ID IN (" + IDs + ")" + GetArrOfficSQLTxt(arrangoff);
            ...
            if (norecs > 0)
                for (int j = 0; j < norecs; j++)
                    if (dnzs.Tables[0].Rows[j][0].ToString() != "" && dnzs.Tables[0].Rows[j][0].ToString() != null)
                    {
                        ...
                        catch (Exception ex) { B = -1; var timestamp = UpdateLog(DateTime.Now, "Make track no -1=not clear what it is" + ex.Message, true, ConfigRepository.Instance()["dlcm_TempPath"], "", "DLCManager", null, null); }
                        ...
                        v += "_" + (dnzs.Tables[0].Rows[j][7].ToString() == "True" ? "Alternate" : "NoAlternate");
                        v += "_" + (dnzs.Tables[0].Rows[j][8].ToString() == "Yes" ? "Favorite" : "NoFavorite");
                        v += "_" + (dnzs.Tables[0].Rows[j][9].ToString() == "Yes" ? "Slide" : "NoSlide");
                        v += "_" + (dnzs.Tables[0].Rows[j][10].ToString() == "440" ? "A440" : "NotA440");
                        v += "_" + (dnzs.Tables[0].Rows[j][11].ToString() == "0" ? "NoCapo" : "Capo");
        ...
        public void AddAlternativeAudioTracksAvailability(string ID)
        {
            ...
            var norecs = GetNoRec(dnzs, cnb, cnc);
            if (norecs > 0)
                for (int j = 0; j < norecs; j++)
            ...
            if (GroupChanged)
            {
                ...
                for (int j = 0; j < chbx_AllGroups.Items.Count; j++)
                {
                    ...
                    if (!found && chbx_AllGroups.GetItemChecked(j))
                    {
                        ...
                        if (noOfRec > 1) if (grpord != dgs.Tables[0].Rows[0].ItemArray[0].ToString())
        ...
        public void SaveRecord()
        {
            ...
            if (databox.SelectedCells.Count > 0 && txt_ID.Text != "")
            {
                ...
                //save fovorite
                var idd = ""; var uc = ""; var upcmd = "Update Arrangements Set Favorite = \"Yes\" WHERE CDLC_ID=";
                idd = ""; if (chbx_LeaF1.Checked) idd = GTAS(chbx_LeaF1, toolTip11, ":"); else if (chbx_LeaF2.Checked) idd = GTAS(chbx_LeaF2, toolTip11, ":"); else if (chbx_LeaF3.Checked) idd = GTAS(chbx_LeaF3, toolTip11, ":");
                var upcmdLea = upcmd + idd + ";"; DataSet dr = new DataSet(); if (idd != "") dr = UpdateDB("Arrangements", upcmdLea, cnb, cnc);
                idd = ""; if (chbx_ComF1.Checked) idd = GTAS(chbx_ComF1, toolTip11, ":"); else if (chbx_ComF2.Checked) idd = GTAS(chbx_ComF2, toolTip11, ":"); else if (chbx_ComF3.Checked) idd = GTAS(chbx_ComF3, toolTip11, ":");
                var upcmdCom = upcmd + idd + ";"; DataSet dv = new DataSet(); if (idd != "") dv = UpdateDB("Arrangements", upcmdCom, cnb, cnc);
                idd = ""; if (chbx_RhyF1.Checked) idd = GTAS(chbx_RhyF1, toolTip11, ":"); else if (chbx_RhyF2.Checked) idd = GTAS(chbx_RhyF2, toolTip11, ":"); else if (chbx_RhyF3.Checked) idd = GTAS(chbx_RhyF3, toolTip11, ":");
                var upcmdRhy = upcmd + idd + ";"; DataSet dz = new DataSet(); if (idd != "") dz = UpdateDB("Arrangements", upcmdRhy, cnb, cnc);
                idd = ""; if (chbx_BasF1.Checked) idd = GTAS(chbx_BasF1, toolTip11, ":"); else if (chbx_BasF2.Checked) idd = GTAS(chbx_BasF2, toolTip11, ":"); else if (chbx_BasF3.Checked) idd = GTAS(chbx_BasF3, toolTip11, ":");
                var upcmdBass = upcmd + idd + ";"; DataSet dn = new DataSet(); if (idd != "") dn = UpdateDB("Arrangements", upcmdBass, cnb, cnc);
                ...
                command.CommandText += "EoFPath = @param140, ";
                command.CommandText += "Found_on_CF = @param141, ";
                command.CommandText += "Uploaded_on_CF = @param142, ";
                command.CommandText += "Uploaded_on_CAPIRO = @param143, ";
                command.CommandText += "FilePath_on_CAPIRO = @param144, ";
                command.CommandText += "Spotify_Song_ID = @param145, ";
                command.CommandText += "CF_Author = @param146, ";
                command.CommandText += "Has_Slide = @param147, ";
                command.CommandText += "BasedOn_RB = @param148, ";
                command.CommandText += "Has_Alternate_Track = @param149, ";
                command.CommandText += "AI_TrackAvail = @param150, ";
                command.CommandText += "Available_Old = @param151,";
                command.CommandText += "Is_Censored = @param152,";
                command.CommandText += "Has_Acoustic = @param153";
                ...
                command.Parameters.AddWithValue("@param32", txt_MultiTrackType.Text.Replace(" (Audio Available)", ""));// databox.Rows[i].Cells["MultiTrack_Version"].Value.ToString());
                ...
                command.Parameters.AddWithValue("@param101", txt_DuplicateOf.Text == "0" ? txt_Duplicate_Of.Text : txt_DuplicateOf.Text);
                ...
                command.Parameters.AddWithValue("@param131", txt_BasedOn_GP.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param132", txt_PackingDate.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param133", txt_UpdateVersionDate.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param134", txt_FilesMissingIssues.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param135", chbx_A_IsMetalCover.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param136", chbx_A_IsUkulele.Checked ? "Yes" : "No");
                ...
                command.Parameters.AddWithValue("@param139", chbx_A440.CheckState.ToString() == "Unchecked" ? "NonStnd" :
                    (chbx_A440.CheckState.ToString() == "440Hz" ? "NonStnd" : "Different"));
                command.Parameters.AddWithValue("@param140", txt_EoFPath.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param141", chbx_Found_on_CF.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param142", chbx_Uploaded_on_CF.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param143", chbx_Found_on_CAPIRO.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param144", txt_filePathonself.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param145", txt_SpotifySongID.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param146", txt_CF_Author.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param147", chbx_A_IsSlide.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param148", txt_BasedOn_RB.Text ?? DBNull.Value.ToString());
                command.Parameters.AddWithValue("@param149", chbx_AltTrack.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param150", chbx_AI_TrackAvail.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param151", chbx_Avail_Old.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param152", chbx_A_IsCensored.Checked ? "Yes" : "No");
                command.Parameters.AddWithValue("@param153", chbx_A_HasAcoustic.Checked ? "Yes" : "No");
        ...
        public int Populate(ref DataGridView DataGridView, ref BindingSource bs)
        {
            ...
            if (dssx.Tables.Count > 0)
            {
                ...
                {
                    var t = SearchCmd.Replace(c("dlcm_SearchFields"), "*").Replace("ORDER BY " + c("dlcm_OrderOfFields"), "");
                    MessageBox.Show("Zero/NO " + " Records/Songs Found\n"
                        + (cmb_Filter.Text == "" ? ", when SQL searching by: " + t + "."
                        : ", using the Filter : \"" + cmb_Filter.Text + "\n\n(sample of the search-ed IDs result of the filter\n i.e." + t + ")"));
                }
            }
            else MessageBox.Show("No Records/Songs (Imported/Found)");
        ...
        private void Update_Selected()
        {
            ...
            catch (Exception ex) { var tsst = "Error ..." + ex.Message; timestamp = UpdateLog(timestamp, tsst, false, c("dlcm_TempPath"), "", "", null, null); }
        ...
        private async void cbx_Format_SelectedValueChanged(object sender, EventArgs e)
        {
            txt_PackFTPFldNOK.Text = "OK";
            txt_PackFTPFldNOK.ForeColor = System.Drawing.Color.Green;
            if (chbx_Format.Text == "PS3_US")
            {
                txt_FTPPath.Text = c("dlcm_FTPUS");
                chbx_PS3HAN.Enabled = true;
                txt_PackFTPFldNOK.Text = FTPAvail(txt_FTPPath.Text);
                txt_PackFTPFldNOK.ForeColor = txt_PackFTPFldNOK.Text == "OK" ? System.Drawing.Color.Green : System.Drawing.Color.Red;
            }
            else
                            ...
                            if (chbx_Format.Text == "PC")
                            {
                                ...
                                if (chbx_Format.Text == "Mac" || chbx_Format.Text == "ImportFolder")
                                {
                                    ...
                                    if (chbx_Format.Text == "ImportFolder")
                                    {
                                        ...
                                        if (chbx_Format.Text == "PS4")
                                        {
                                            txt_FTPPath.Text = DirectoryExists(c("general_rs2014path")) && c("general_rs2014path").IndexOf(":\\") >= 0 ? c("general_rs2014path") : c("dlcm_PS4");/// c("dlcm_RocksmithDLCPath");               
                                        }
                                        else
                                        {
                                            txt = "No PC/Mac/PS4 also-to-save Folder available:";
                                            txt_FTPPath.Text = "";
                                            txt_PackFTPFldNOK.Text = "NOK";
                                            txt_PackFTPFldNOK.ForeColor = System.Drawing.Color.Red;
                                        }
        ...
        private void savesettings()
        {
            ...
            ConfigRepository.Instance()["dlcm_AdditionalManipul102"] = chbx_Clean.Checked ? "Yes" : "No";
            ConfigRepository.Instance()["dlcm_AdditionalManipul128"] = chbx_SKU.Checked ? "Yes" : "No";
            ConfigRepository.Instance()["dlcm_AdditionalManipul129"] = chbx_Hide.Checked ? "Yes" : "No";
            ...
            ConfigRepository.Instance()["dlcm_AdditionalManipul64"] = chbx_Last_Packed.Checked ? "Yes" : "No";
            ConfigRepository.Instance()["dlcm_AdditionalManipul127"] = chbx_FN.Checked ? "Yes" : "No";
        ...
        private void btn_PlayPreview_Click(object sender, EventArgs e)
        {
            ...
            {
                var tsst = "Erro ...btn_PlayPreview_Click" + ex.Message; timestamp = UpdateLog(timestamp, tsst, false, c("dlcm_TempPath"), "", "", null, null);
            }

            ProcessStarted = false;
            var startInfo = new ProcessStartInfo
            {
                FileName = Path.Combine(ExtAppWD, "oggdec2.exe"),
                WorkingDirectory = ExtAppWD
            };
        ...
        //for conversion the wwise needs to be downlaoded from https://www.audiokinetic.com/download/?id=2014.1.6_5318 or https://www.audiokinetic.com/download/?id=2013.2.10_4884
        {
            ...
            var startInfo = new ProcessStartInfo
            {
                FileName = Path.Combine(ExtAppWD, "oggcut.exe"),
                WorkingDirectory = ExtAppWD.Replace("external_tools", "")
            };
        ...
        private void SetSelAndExtraAttrib(bool InclSelected, bool InclBeta, bool InclLanguage, bool InclBroken, bool InclGroups, string LyricsLanguage, string Status, string invert, CheckedListBox AllGroups)
        {
            ...
            {
                var rtz = "";
                //identify all already selected Groups as to not to override/duplicate them
                var sel = "SELECT CDLC_ID, Groupz, ID FROM Groups WHERE Type =\"DLC\" AND VAL(CDLC_ID) " + invert + " IN (" + SearchCmd.Replace("  ", " ").Replace(SearchFields.Replace("  ", " "), "ID ").Replace(";", "") + ")";
                DataSet grp = new DataSet(); grp = SelectFromDB("Groups", sel, "", cnb, cnc);
                var noOfRecs = GetNoRec(grp, cnb, cnc);//grp.Tables.Count == 0 ? 0 : grp.Tables[0].Rows.Count;
                for (var k = 0; k < AllGroups.Items.Count; k++)
                    if (AllGroups.GetItemChecked(k)) rtz += chbx_AllGroups.Items[k].ToString() + ",";
                result1 = MessageBox.Show("Including Setting each song to the groups selected for current song (i.e.: " + rtz + ")?", MESSAGEBOX_CAPTION, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                if (result1 == DialogResult.Yes)
                {
        ...
        private void btn_Duplicate_Click(object sender, EventArgs e)
        {
            ...
            try //Copy dir
            {
                ...
                var insertcmdd = "Song_Title, Song_Title_Sort, Album, Artist, Artist_Sort, Album_Year, AverageTempo, Volume, Preview_Volume, AlbumArtPath, AudioPath," +
                    ...
                    " Has_ShowLights, Has_JVocals, Is_Medley, Is_MultiStrings, BasedOn_GP, Album_ArtPathOrig, Is_Deluxe, Is_GreatestHits, Is_Midi, Is_GameSoundtrack, Is_TVTheme, Is_AmateurCover, Is_MetalCover" +
                    ", Is_Ukulele, Has_Alternate_Audio, Has_Alternate_Lyrics, A440TunningFrecv, Uploaded_on_CF, Found_on_CF, Uploaded_on_CAPIRO, FilePath_on_CAPIRO" +
                    ", CF_Author, LastConversionDateTime, Has_Alternate_Track, Has_Slide, BasedOn_RB, AI_TrackAvail, Is_Censored, Has_Acoustic";
                ...
                var insertvalues = "SELECT Song_Title+\" alt" + max.ToString() + "\", Song_Title_Sort+\" alt" + max.ToString() + "\", Album, Artist, Artist_Sort, Album_Year," +
                    ...
                    ",EoFPath, Is_Karaoke, Is_Cover, Has_Featuring, Is_Demo, Is_Remix, BasedOn_Youtube, BasedOn_CF, BasedOn_Tabs, ToDos, ToneDetails, PackageDetails, PackingDate, UpdateVersionDate, Has_Capo," +
                    " Has_ShowLights, Has_JVocals, Is_Medley, Is_MultiStrings, BasedOn_GP, \"" + AlbumOrigArtPath + "\", Is_Deluxe, Is_GreatestHits, Is_Midi, Is_GameSoundtrack" +
                    ", Is_TVTheme, Is_AmateurCover, Is_MetalCover, Is_Ukulele, Has_Alternate_Audio, Has_Alternate_Lyrics, A440TunningFrecv, Uploaded_on_CF, Found_on_CF" +
                    ", Uploaded_on_CAPIRO, FilePath_on_CAPIRO, CF_Author, LastConversionDateTime, Has_Alternate_Track, Has_Slide, BasedOn_RB, AI_TrackAvail, Is_Censored, Has_Acoustic" +
                ...
                insertcmdd = "Arrangement_Name, CDLC_ID, Bonus, JSONFilePath, XMLFilePath, XMLFile_Hash, ScrollSpeed, Tunning, Rating, PlaythroughYBLink, CustomsForge_Link," +
                    ...
                    "SNGFileHash, Has_Sections, Comments, Start_Time, CleanedXML_Hash, Json_Hash, Part, MaxDifficulty, OrigSongTrack, PrimaryTrack, Favorite, Broken, Official" +
                    ", PersistentID, CapoFret, A440TunningFrecv, Slide, Alternate";
                insertvalues = "SELECT Arrangement_Name, " + CDLC_ID + ", Bonus, \"" + destination_dir + "\\manifests\\\"+right(JSONFilePath,len(JSONFilePath)-" +
                    ...
                    " Json_Hash, Part, MaxDifficulty, OrigSongTrack, PrimaryTrack, Favorite, Broken, Official, PersistentID, CapoFret, A440TunningFrecv, Slide" +
                    ", Alternate FROM Arrangements WHERE CDLC_ID = " + txt_ID.Text;
        ...
        private void btn_AddSections_Click(object sender, EventArgs e)
        {
            ...
            var result1 = MessageBox.Show("Use old tool BTM (Yes) or Some Internal Logic (No)", MESSAGEBOX_CAPTION, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
            if (result1 == DialogResult.Yes)
            {
                btm(true);
            }
            else
            {
                ...
                //string array
                //columnn 1 is instrument/track
                //column 2 is time
                //column 3 is fret
                string[,,] nt0 = new string[10, 3000, 2];//consolidation of all other notes timings
                string[,,] nt1 = new string[10, 3000, 2];
                string[,,] nt2 = new string[10, 3000, 2];
                string[,,] nt3 = new string[10, 3000, 2];
                string[,,] nt4 = new string[10, 3000, 2];
                string[,,] nt5 = new string[10, 3000, 2];
                string[,,] nt6 = new string[10, 3000, 2];
                string[,] chords = new string[2000, 7];
                string[,] arg = new string[10, 7];
                float min = float.Parse("100");

                bool superOrg = false; var i = databox.SelectedCells[0].RowIndex;
                var xml = ""; var Folder_Name = databox.Rows[i].Cells["Folder_Name"].Value.ToString();

                DataSet dbn = new DataSet(); dbn = SelectFromDB("Arrangements", "SELECT ArrangementType, XMLFilePath, JSONFilePath FROM Arrangements WHERE ArrangementType IN (\"Bass\",\"Guitar\") AND CDLC_ID=\"" + txt_ID.Text + "\"" + GetArrOfficSQLTxt(arrangoff), "", cnb, cnc);
                var norec = dbn.Tables.Count < 1 ? 0 : dbn.Tables[0].Rows.Count;
                var t = 1; float snglng = 0;
                if (norec > 0)
                    for (int k = 0; k < norec; k++)
                    {
                        xml = dbn.Tables[0].Rows[k][1].ToString();//add items
                        var json = dbn.Tables[0].Rows[k][2].ToString();
                        if (dbn.Tables[0].Rows[k][0].ToString().ToLower() != "bass" && dbn.Tables[0].Rows[k][0].ToString().ToLower() != "guitar") continue;

                        Platform platform = Folder_Name.GetPlatform(); //string[] xmlFilez = new string[c("dlcm_maxsongsinDLCM").ToInt32()]; bool done = true; var countd = 0;                        
                        ...
                        {
                            xmlContent = Song2014.LoadFromFile(xml);
                            snglng = xmlContent.SongLength;
                            arg[k, 0] = dbn.Tables[0].Rows[k][0].ToString().ToLower();
                            //Save a copy
                            //if (!File.Exists(xml + ".old8")) File.Copy(xml, xml + ".old8", false);
                            //else File.Copy(xml, xml + ".old8", true);
                            //var json = jsons;
                            //if (!File.Exists(json + ".old8")) File.Copy(json, json + ".old8", false);
                            //else { File.Copy(json + ".old8", json, true); }
                            //if (xmlContent.Arrangement.ToLower() == "bass" || xmlContent.Arrangement.ToLower() == "guitar")
                            //{
                            ...
                            {  //Save a copy
                                ...
                                xml = xml + ".old8";
                                json = json + ".old8";
                                RemoveDD(Folder_Name, chbx_Original.Checked ? "Yes" : "No", xml, platform, false, false, chbx_UseInternalDDRemovalLogic.Checked ? "Yes" : "No");
                                xmlContent = Song2014.LoadFromFile(xml);
                            }
                            ...
                            if (xmlContent.Levels[0].Chords.Length > 0)
                                ...
                                {
                                    //if (xmlContent.Levels[0].Notes[n].Time > xmlContent.Levels[0].Chords[m].Time)
                                    if (chords[xmlContent.Levels[0].Chords[m].ChordId, 1] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt1[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt1[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 1]; }
                                    if (chords[xmlContent.Levels[0].Chords[m].ChordId, 2] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt2[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt2[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 2]; }
                                    if (chords[xmlContent.Levels[0].Chords[m].ChordId, 3] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt3[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt3[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 3]; }
                                    if (chords[xmlContent.Levels[0].Chords[m].ChordId, 4] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt4[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt4[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 4]; }
                                    if (chords[xmlContent.Levels[0].Chords[m].ChordId, 5] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt5[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt5[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 5]; }
                                    if (chords[xmlContent.Levels[0].Chords[m].ChordId, 6] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt6[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt6[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 6]; }
                                    //if (xmlContent.Levels[0].Notes[n].Fret == 1) nt1[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString();
                                    t++;
                                }
                            if (xmlContent.Levels[0].Notes.Length > 0)
                                for (var n = 0; n < xmlContent.Levels[0].Notes.Length; n++)
                                {
                                    if (xmlContent.Levels[0].Notes[n].String == 0) { nt0[k, n, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt1[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt1[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                    else if (xmlContent.Levels[0].Notes[n].String == 1) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt2[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt2[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                    else if (xmlContent.Levels[0].Notes[n].String == 2) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt3[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt3[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                    else if (xmlContent.Levels[0].Notes[n].String == 3) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt4[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt4[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                    else if (xmlContent.Levels[0].Notes[n].String == 4) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt5[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt5[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                    else if (xmlContent.Levels[0].Notes[n].String == 5) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt6[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt6[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                    t++;
                                }

                            //sort
                            for (var n = 1; n <= xmlContent.Levels[0].Notes.Length + xmlContent.Levels[0].Chords.Length - 1; n++)
                                for (var m = n + 1; m <= xmlContent.Levels[0].Notes.Length + xmlContent.Levels[0].Chords.Length; m++)
                                {
                                    //var timesn = nt1[k, n, 0]; timesn = timesn is not null && nt2[k, n, 0] is not null? timesn.Replace(nt2[k, n, 0], ""):null + nt2[k, n, 0]; timesn = timesn is not null && nt3[k, n, 0] is not null ? timesn.Replace(nt3[k, n, 0], ""): null + nt3[k, n, 0]; timesn = timesn is not null && nt4[k, n, 0] is not null ? timesn.Replace(nt4[k, n, 0], ""): null + nt4[k, n, 0]; timesn = timesn is not null && nt5[k, n, 0] is not null ? timesn.Replace(nt5[k, n, 0], ""): null + nt5[k, n, 0]; timesn = timesn is not null && nt6[k, n, 0] is not null ? timesn.Replace(nt6[k, n, 0], ""): null + nt6[k, n, 0];
                                    //var timesm = nt1[k, m, 0]; timesm = timesm is not null && nt2[k, m, 0] is not null? timesm.Replace(nt2[k, m, 0], ""):null + nt2[k, m, 0]; timesm = timesm is not null && nt3[k, m, 0] is not null ? timesm.Replace(nt3[k, m, 0], ""): null + nt3[k, m, 0]; timesm = timesm is not null && nt4[k, m, 0] is not null ? timesm.Replace(nt4[k, m, 0], ""): null + nt4[k, m, 0]; timesm = timesm is not null && nt5[k, m, 0] is not null ? timesm.Replace(nt5[k, m, 0], ""): null + nt5[k, m, 0]; timesm = timesm is not null && nt6[k, m, 0] is not null ? timesm.Replace(nt6[k, m, 0], ""): null + nt6[k, m, 0];
                                    ...
                                    //    if (nt1[k, n, 0] is not null && nt1[k, m, 0] is not null) { if (float.Parse(nt1[k, n, 0]) > float.Parse(nt1[k, m, 0])) { var z = nt1[k, n, 1]; nt1[k, n, 1] = nt1[k, m, 1]; nt1[k, m, 1] = z; var u = nt1[k, n, 0]; nt1[k, n, 0] = nt1[k, m, 0]; nt1[k, m, 0] = u; } }
                                    //    else if (nt2[k, n, 0] is not null && nt2[k, m, 0] is not null) { if (float.Parse(nt2[k, n, 0]) > float.Parse(nt2[k, m, 0])) { var z = nt2[k, n, 1]; nt2[k, n, 1] = nt2[k, m, 1]; nt2[k, m, 1] = z; var u = nt2[k, n, 0]; nt2[k, n, 0] = nt2[k, m, 0]; nt2[k, m, 0] = u; } }
                                    //    else if (nt3[k, n, 0] is not null && nt3[k, m, 0] is not null) { if (float.Parse(nt3[k, n, 0]) > float.Parse(nt3[k, m, 0])) { var z = nt3[k, n, 1]; nt3[k, n, 1] = nt3[k, m, 1]; nt3[k, m, 1] = z; var u = nt3[k, n, 0]; nt3[k, n, 0] = nt3[k, m, 0]; nt3[k, m, 0] = u; } }
                                    //    else if (nt4[k, n, 0] is not null && nt4[k, m, 0] is not null) { if (float.Parse(nt4[k, n, 0]) > float.Parse(nt4[k, m, 0])) { var z = nt4[k, n, 1]; nt4[k, n, 1] = nt4[k, m, 1]; nt4[k, m, 1] = z; var u = nt4[k, n, 0]; nt4[k, n, 0] = nt4[k, m, 0]; nt4[k, m, 0] = u; } }
                                    //    else if (nt5[k, n, 0] is not null && nt5[k, m, 0] is not null) { if (float.Parse(nt5[k, n, 0]) > float.Parse(nt5[k, m, 0])) { var z = nt5[k, n, 1]; nt5[k, n, 1] = nt5[k, m, 1]; nt5[k, m, 1] = z; var u = nt5[k, n, 0]; nt5[k, n, 0] = nt5[k, m, 0]; nt5[k, m, 0] = u; } }
                                    //    else if (nt6[k, n, 0] is not null && nt6[k, m, 0] is not null) { if (float.Parse(nt6[k, n, 0]) > float.Parse(nt6[k, m, 0])) { var z = nt6[k, n, 1]; nt6[k, n, 1] = nt6[k, m, 1]; nt6[k, m, 1] = z; var u = nt6[k, n, 0]; nt6[k, n, 0] = nt6[k, m, 0]; nt6[k, m, 0] = u; } }
                                }

                            //try
                            //get the smallest space between notes

                            //for (var k = 1; m <= norec; m++)
                            for (var n = 1; n <= t; n++)
                            {
                                var timesn1 = nt0[k, n, 0];
                                var timesn2 = nt0[k, n + 1, 0];
                                //var timesm = nt1[k, m, 0] + nt2[k, m, 0] + nt3[k, m, 0] + nt4[k, m, 0] + nt5[k, m, 0] + nt6[k, m, 0];
                                if (timesn1 is not null && timesn2 is not null)
                                    if (float.Parse(timesn1) >= 0 && float.Parse(timesn2) >= 0)
                                        if (float.Parse(timesn2) - float.Parse(timesn1) > 0)
                                        { if (float.Parse(timesn2) - float.Parse(timesn1) < min) min = float.Parse(timesn2) - float.Parse(timesn1); }
                                //else if (float.Parse(timesn1) >= 0&& float.Parse(timesn2) >= 0) { if (float.Parse(timesn1) - float.Parse(timesn2) < min) min = float.Parse(timesn2) - float.Parse(timesn1); }
                                //else if (float.Parse(nt2[m, n, 0]) >= 0) { if (float.Parse(nt2[m, n, 0]) - float.Parse(nt2[m, n + 1, 0]) < min) min = float.Parse(nt2[m, n, 0]) - float.Parse(nt2[m, n + 1, 0]); }
                                //else if (float.Parse(nt3[m, n, 0]) >= 0) { if (float.Parse(nt3[m, n, 0]) - float.Parse(nt3[m, n + 1, 0]) < min) min = float.Parse(nt3[m, n, 0]) - float.Parse(nt3[m, n + 1, 0]); }
                                //else if (float.Parse(nt4[m, n, 0]) >= 0) { if (float.Parse(nt4[m, n, 0]) - float.Parse(nt4[m, n + 1, 0]) < min) min = float.Parse(nt4[m, n, 0]) - float.Parse(nt4[m, n + 1, 0]); }
                                //else if (float.Parse(nt5[m, n, 0]) >= 0) { if (float.Parse(nt5[m, n, 0]) - float.Parse(nt5[m, n + 1, 0]) < min) min = float.Parse(nt5[m, n, 0]) - float.Parse(nt5[m, n + 1, 0]); }
                                //else if (float.Parse(nt6[m, n, 0]) >= 0) { if (float.Parse(nt6[m, n, 0]) - float.Parse(nt6[m, n + 1, 0]) < min) min = float.Parse(nt6[m, n, 0]) - float.Parse(nt6[m, n + 1, 0]); }
                            }
                        }
                        catch (Exception ex)
                        {
                            var tsst = "Error at consolid nostes 4 quick display ..." + ex.Message; UpdateLog(DateTime.Now, tsst, false, "", "", "", null, null);
                        }
                    }
                //add header: song name, tunning
                txt = "Song Title: " + txt_Title.Text;
                txt += "\nArtist: " + txt_Artist.Text;
                txt += "\nAlbum: " + txt_Album.Text;
                txt += "\nTuning: " + txt_Tuning.Text;
                txt += btn_PitchShift.Enabled ? "\nDetuning with Digitec Pedal: " + toolTip1.GetToolTip(btn_PitchShift).Replace("\n", "\n\t\t\t") : "";
                txt += "\nSpace between notes: " + min;
                txt += "\nMarker: every 10s";
                txt += "\nsong Lenght: " + snglng;
                //add content of tracks
                //with 10s tracker bar
                //with track headers
                txt += "\n"; float z = 0; var done = false;
                for (int k = 0; k < norec; k++)
                {
                    txt += "\n" + k + "/" + norec + " " + arg[k, 0].ToUpper() + "\n";
                    z = 0; done = false; var u = 10;
                    do
                    {
                        if (((z + min) / u) > 1)
                        { txt += "|"; u += 10; }/*done = true; */
                    ...
                    for (int n = 1; n <= t; n++)
                    {
                        do
                        {
                            if (nt1[k, n, 0] is not null)
                            {
                                if (float.Parse(nt1[k, n, 0]) >= z + min) { txt += nt1[k, n, 1]; done = true; }
                                else txt += "-";
                            }
                            else txt += "-";
                            z = z + min;
                    ...
                    for (int n = 1; n <= t; n++)
                    {
                        do
                        {
                            if (nt2[k, n, 0] is not null)
                            {
                                if (float.Parse(nt2[k, n, 0]) >= z + min) { txt += nt2[k, n, 1]; done = true; }
                                else txt += "-";
                            }
                            else txt += "-";
                            z = z + min;
                    ...
                    for (int n = 1; n <= t; n++)
                    {
                        do
                        {
                            if (nt3[k, n, 0] is not null)
                            {
                                if (float.Parse(nt3[k, n, 0]) >= z + min) { txt += nt3[k, n, 1]; done = true; }
                                else txt += "-";
                            }
                            else txt += "-";
                            z = z + min;
                    ...
                    for (int n = 1; n <= t; n++)
                    {
                        do
                        {
                            if (nt4[k, n, 0] is not null)
                            {
                                if (float.Parse(nt4[k, n, 0]) >= z + min) { txt += nt4[k, n, 1]; done = true; }
                                else txt += "-";
                            }
                            else txt += "-";
                            z = z + min;
                    ...
                    for (int n = 1; n <= t; n++)
                    {
                        do
                        {
                            if (nt5[k, n, 0] is not null)
                            {
                                if (float.Parse(nt5[k, n, 0]) >= z + min) { txt += nt5[k, n, 1]; done = true; }
                                else txt += "-";
                            }
                            else txt += "-";
                            z = z + min;
                    ...
                    for (int n = 1; n <= t; n++)
                    {
                        do
                        {
                            if (nt6[k, n, 0] is not null)
                            {
                                if (float.Parse(nt6[k, n, 0]) >= z + min) { txt += nt6[k, n, 1]; done = true; }
                                else txt += "-";
                            }
                            else txt += "-";
                            z = z + min;
                ...
                //    }
                ErrorWindow frm1 = new ErrorWindow(txt, "", "Quick Tab display of song-tracks & Notes", true, false, false, "OK", "", "", true);
                frm1.ShowDialog();
                //if (frm1.B2) return true;
                //else return false;
            }
        }
        ...
        void UpdateGroups()
        {
            try
            {
                ...
                var nocrec = GetNoRec(ddg, cnb, cnc);// getnorec ddg.Tables[0].Rows.Count;
                ...
                if (nocrec > 0 && lastrow <= nocrec) min = ddg.Tables[0].Rows[lastrow][2].ToString().Replace(" ", "");
        ...
        private void btn_GarageBand_Click(object sender, EventArgs e)
        {
            var startInfo = new ProcessStartInfo
            {
                FileName = Path.Combine(AppWD, "DLCManager\\external_tools\\oggdec.exe"),
        ...
        private void btn_ChangeLyrics_Click(object sender, EventArgs e)
        {
            ...
            using (var fbd = new OpenFileDialog())
            {
                ...
                ApplyLyric(fbd.FileName, databox.Rows[i].Cells["Folder_Name"].Value.ToString(), cnb,
                    txt_Platform.Text.ToString().Replace("PC", "Pc"), txt_ID.Text, chbx_Lyrics.Checked, cnc, false);
                ...
                chbx_IsImprovedWithDM.Checked = true;
                chbx_IsImprovedWithDM.Visible = true;
            }
        }
        ...
        public void ReadGameLibrary()
        {
            ...
            string[] dates = new string[20000];
            string[] size = new string[20000]; ConfigRepository.Instance()["dlcm_FTPstatus"] = "OK";
            ...
            foreach (string fileName in fileEntrie)
            {
                ...
                if (fileName.Contains("root"))
                {
                    ...
                    fileEntries[l] = args[16].Replace(";", "");
                    dates[l] = args[14] + "/" + args[14] + " " + args[15];
                    size[l] = args[12];
                }
                else
                {
            ...
            if (fileEntries != null)
            {
                ...
                foreach (string fileName in fileEntries)
                {
                    ...
                    pB_ReadDLCs.CreateGraphics().DrawString(tst, new System.Drawing.Font("Arial", 7, FontStyle.Bold), System.Drawing.Brushes.Blue, new PointF(1, pB_ReadDLCs.Height / 4));

                    if (fileName.IndexOf("s1compatibility") > 0)
                        continue;
                    z++; logmss = z + "/" + l + "Reading... " + fileName + "..."; timestamp = UpdateLog(timestamp, logmss, false, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);

                    DataSet dfs = new DataSet(); dfs = SelectFromDB("Pack_AuditTrail", "SELECT CDLC_ID, PackDate FROM Pack_AuditTrail WHERE FileName=\"" + Path.GetFileName(fileName) + "\" ORDER BY ID DESC;", "", cnb, cnc);
                    ...
                    else
                    {
                        ...
                        {
                            var fn = "";
                            DataSet dfg = new DataSet(); dfg = SelectFromDB("Main", "SELECT ID, Remote_Path, \"\", ID, Artist, Album, Song_Title, Album_Year, File_Creation_Date FROM Main WHERE ID=" + dlcid + ";", "", cnb, cnc);
                            //var ptz = new System.IO.FileInfo(fileName);
                            var norek = GetNoRec(dfg, cnb, cnc);//dfs.Tables[0].Rows.Count;
                            if (norek == 0) MessageBox.Show("No DLC found");
                            ...
                            DialogResult result1 = MessageBox.Show("Duplicate DLC has been found!\n\by searching for the filename " + "\ninfo: " + info
                                + ":\n\nChose which to immediately delete:\n\n(Yes) 1. (Pack_AuditTrail details)"
                                + dfs.Tables[0].Rows[0].ItemArray[0].ToString() + ")\n\n(No) 2." +/*dfg.Tables[0].Rows[0].ItemArray[1].ToString() + " (" + dfg.Tables[0].Rows[0].ItemArray[8].ToString() */
                                Path.GetFileName(fileName) + " (" + dates[z] + ")\n\n(Cancel) 3. Ignore\n\n(from " + pathx + ")", MESSAGEBOX_CAPTION, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
        ...
        {
            var NoRencodes = 0; var Noorphaned = 0; var NoOrphanSpotifyArt = 0; ; var NoOrphanD = 0;
            ...
            var dlg = pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + " Do you want to process all Audio/WEM files, check the individual bitrate then downstream to desired?\n" +
                "Note some WEM conv raises the bitrate of the audio(but not the file size) so some wems will always get re-Encoded when checking.";
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                ...
                for (var i = 0; i < noOfRecs; i++)
                {
                    ...
                    var xx = "";
                    if (File.Exists(paath)) xx = paath;
                    else xx = Path.Combine(AppWD, "MediaInfo_CLI_Windows_x64", "MediaInfo.exe");
            ...
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                ...
                string[] del = new string[c("dlcm_maxsongsinDLCM").ToInt32()]; var fldl = "";
                foreach (string s in dirs)
                {
                    ...
                    {
                        NoOrphanD++;
                        del[NoOrphanD] = s; del[0] = NoOrphanD.ToString(); fldl += "\n" + s;
                ...
                if (NoOrphanD > 0)
                    if (ShowDialogue(dlg + fldl, " Check ", NoOrphanD, "delete (more info in logs)", 2))
                        for (var j = 0; j < del[0].ToInt32(); j++)
                        {
                            ...
                            timestamp = UpdateLog(timestamp, "\n\tDeleted orphaned folder:" + del[j], true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                        }
            ...
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                ...
                foreach (string s in dirs)
                {
                    if (s.IndexOf("0_old") <= 0 && s.IndexOf("0_repacked") <= 0 && s.IndexOf("0_duplicate") <= 0 && s.IndexOf("0_dlcpacks") <= 0
                        && s.IndexOf("0_broken") <= 0 && s.IndexOf("0_albumCovers") <= 0 && s.IndexOf("0_log") <= 0 && s.IndexOf("0_archive") <= 0
                        && s.IndexOf("0_data") <= 0 && s.IndexOf("0_temp") <= 0 && s.IndexOf("0_to_import") <= 0 && s.IndexOf("0_export") <= 0 && s.IndexOf("0_intheworks") <= 0)
                    {
                        ...
                        //{
                        Noorphaned++;
                        del[Noorphaned] = s; del[0] = Noorphaned.ToString(); fldl += "\n" + s;
                ...
                pB_ReadDLCs.Value = 0; pB_ReadDLCs.Maximum = Noorphaned;
                if (Noorphaned > 0)
                    if (ShowDialogue(dlg + fldl, " Check ", Noorphaned, "delete (more info in logs)", 2))
                        for (var j = 0; j < del[0].ToInt32(); j++)
                        {
            ...
            o++; pB_ReadDLCs.Maximum = max; pB_ReadDLCs.Value = o; timestamp = UpdateLog(timestamp, o + "/" + max + " check orphaned songs in _old", true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            dlg = pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + " Do you want to check for orphaned songs in _old?";
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                ...
                string[] cpyd = new string[c("dlcm_maxsongsinDLCM").ToInt32()]; var fldl = "";
                foreach (string s in dirs)
                {
                    ...
                    {
                        NoExtra++;
                        cpys[NoExtra] = s; cpys[0] = NoExtra.ToString(); fldl += "\n" + s;
                ...
                if (NoExtra > 0)
                    if (ShowDialogue(dlg + fldl, " check ", NoExtra, "copy safely (more info in logs)", 2))
                        for (var j = 0; j < NoExtra; j++)
                        {
                            if (cpys[j] is null) continue;
            ...
            ////Clean dead CDLC_ID
            //o++; pB_ReadDLCs.Maximum = max; pB_ReadDLCs.Value = o; timestamp = UpdateLog(timestamp, o + "/" + max + " check Groups for orphaned CDLC songs or groups", true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            ...
            //7.check Groups for orphaned entries
            ...
            //8.Delete orphaned Arrangements
            o++; pB_ReadDLCs.Maximum = max; pB_ReadDLCs.Value = o; timestamp = UpdateLog(timestamp, o + "/" + max + " check Arrangements for orphaned entries songs", true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            dlg = pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + " Do you want to check Arrangements for orphaned entries songs?";
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                cmdz = "SELECT Arrangements.ID FROM Arrangements LEFT JOIN Main ON Arrangements.CDLC_ID = Main.ID WHERE Main.ID Is Null";// SELECT * FROM Arrangements LEFT JOIN Main ON Arrangements.CDLC_ID = Main.ID WHERE Main.ID Is Null";
                ...
                cmdz = "DELETE * FROM Arrangements WHERE Arrangements.ID IN (" + cmdz + "";
                ProgressWithText(pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + "Remove " + noOfArrj + " Arrangements for orphaned entries songs", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                if (noOfArrj > 0)
            ...
            //9.Delete orphaned Tones
            o++; pB_ReadDLCs.Maximum = max; pB_ReadDLCs.Value = o; timestamp = UpdateLog(timestamp, o + "/" + max + " check Tones for orphaned entries songs", true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            dlg = pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + " Do you want to check Tones for orphaned entries songs?";
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                cmdz = "SELECT Tones.ID FROM Tones LEFT JOIN Main ON Tones.CDLC_ID = Main.ID WHERE Main.ID Is Null";
                ...
                cmdz = "DELETE * FROM Tones WHERE Tones.ID IN (" + cmdz + "";
                ProgressWithText(pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + "Remove " + noOfArrl + " Tones for orphaned entries songs", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                if (noOfArrl > 0)
            ...
            //10.Delete orphaned tones details
            o++; pB_ReadDLCs.Maximum = max; pB_ReadDLCs.Value = o; timestamp = UpdateLog(timestamp, o + "/" + max + "4 check Tones_GearList details for orphaned entries songs", true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            dlg = pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + " Do you want to check Tones_GearList for orphaned entries songs?";
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                cmdz = "SELECT Tones_GearList.ID FROM Tones_GearList LEFT JOIN Tones ON Tones_GearList.Tone_ID = Tones.ID WHERE Tones.ID Is Null";
                ...
                cmdz = "DELETE * FROM Tones_GearList WHERE Tones_GearList.ID IN (" + cmdz + "";
                ProgressWithText(pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + "Remove " + noOfArrv + " ones_GearList for orphaned entries songs", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                if (noOfArrv > 0)
            ...
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                cmdz = "SELECT CDLC_ID,ID FROM Import_AuditTrail WHERE CDLC_ID in (SELECT str(ID) FROM Main) and (Artist='' OR Artist_Sort='' OR Song_Title='' OR Song_Title_Sort='' OR" +
            ...
            //12.1.check for any missing AUDIO files issues
            o++; pB_ReadDLCs.Maximum = max; pB_ReadDLCs.Value = o; timestamp = UpdateLog(timestamp, o + "/" + max + "4 check Import_AuditTrail for missing Old/At import metadata", true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            dlg = pB_ReadDLCs.Value + "/" + pB_ReadDLCs.Maximum + " Do you want to check and fix Audio file name or file existence or file consistency issues?" +
                "\n\n fyi Broken Exclude Cleanup flag (Brkn checkbox in Pack section) is set to +" + chbx_ExcludeBroken.Checked
                + "\n\nChecks will include if the2 wem and 2 oggs associated with a standard CDLC are existing, have a file-size diff than zero and can be read by Media-Info," +
                " get their details(bitrate etc.) updated, there are no way, too many files wo .bnk coresponding indexer file that could generate and have a normal file-name syntax for ogg or matching .bnk file.";
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                FixAudiofileInconsist(0, pB_ReadDLCs, rtxt_StatisticsOnReadDLCs, maindbpb, timestamp, AppWD, SearchCmd, false, chbx_ExcludeBroken.Checked);
            }
            ...
            if (ShowDialogue(dlg, "", -1, "", 2))
            {
                ...
                for (var i = 0; i < noOfRec; i++)
                {
                    ...
                    if (noOfArrss == 0)
                    {
                        vFilesMissingIssues += " Import_AuditTrail missing (&was fixed; rerun check to remove this note :) )";
                    ...
                    for (var k = 0; k < noOfArr; k++)
                    {
                        ...
                        catch (Exception ex) { var tsst = "Error at file existence ..." + ex.Message; timestamp = UpdateLog(timestamp, tsst, false, c("dlcm_TempPath"), "", "", null, null); }
                    }
            ...
            if (cnt > 0)
            {
                ...
                if (MissingPSARC) MessageBox.Show("Please note that Pack_AuditTrail has unmatched records. Manually decide if they have to be deleted based on Reason field.");
                if (SearchCmd.IndexOf("ORDER BY") < 1) SearchCmd += " ORDER BY " + c("dlcm_OrderOfFields") + " ";
                ...
                MessageBox.Show("Song" + cnt + " with FilesMissing Issues identified"
                    ...
                    + "\n\n3. No. Folders in 0 temporarely left there after (failed?) import: " + Noorphaned
                    ...
                    + "\n\n14. No. orphaned Group entries Deleted: " + NoOGroup
                    + "\n\n15. No. orphaned Arrangements Deleted: " + NoOArg
                    + "\n\n16. No. orphaned Tones entries Deleted: " + NoOTones
                    //+ "\n\n17. No. orphaned Groups DLC Deleted: " + NoOGrp
                    + "\n\n17. No Duplicate Group DLC: " + noOfDLCGrpDpl
                    + "\n\n17. No. orphaned ToneGL Deleted: " + NoOTGL
        ...
        public void btn_FixAudioAll_Click(object sender, EventArgs e)
        {
            ...
            FixAudioAll_Click(netstatus, cnb, AppWD, pB_ReadDLCs, rtxt_StatisticsOnReadDLCs, cancel, SearchCmd, SearchFields, logPath, timestamp, cnc, true);

            Populate(ref databox, ref Main);
            databox.Visible = false; databox.Refresh(); databox.Visible = true;
            pB_ReadDLCs.CreateGraphics().DrawString("Done Fixing Audio Issues for this song.", new Font("Arial", 7, FontStyle.Bold), System.Drawing.Brushes.Blue, new PointF(1, pB_ReadDLCs.Height / 4));
            btn_FixAudioAll.Text = "Fix Audio Issues";
        }
            ...
            bool inclBorken)
        {
            ...
            var cmd = "SELECT ID, AudioPath, audioBitrate, audioSampleRate, audioPreviewPath, Folder_Name, OggPath, oggPreviewPath FROM Main " +
                "WHERE " + (inclBorken ? "" : "(FilesMissingIssues is null AND Is_Broken<>\"Yes\") AND  (Has_Preview=\"No\" OR oggPreviewPath=\"\" OR audioPreviewPath=\"\") AND ") +
                "ID IN(" + SearchCmd.Replace(" * ", " ID ").Replace("; ", "").Replace(SearchFields, "ID") + ") " +
      "" + (c("dlcm_AdditionalManipul55").ToLower() != "yes" ? "" : " AND (VAL(PreviewLenght) > " + float.Parse(c("dlcm_MaxPreviewLenght"), NumberStyles.Float, CultureInfo.CurrentCulture) + ")" +
        (c("dlcm_AdditionalManipul88").ToLower() != "yes" ? "" : " OR (VAL(PreviewLenght) < " + float.Parse(c("dlcm_MinPreviewLenght"), NumberStyles.Float, CultureInfo.CurrentCulture)) + ")");
            ...
            cmd = "SELECT ID, AudioPath, audioBitrate, audioSampleRate, audioPreviewPath, oggPath, oggPreviewPath FROM Main" +
                " WHERE (VAL(audioBitrate) > "
                + (c("dlcm_MaxBitRate")) + " or VAL(audioSampleRate) > " + (c("dlcm_MaxSampleRate")) + ") AND "
                + "ID IN(" + SearchCmd.Replace(" * ", " ID ").Replace("; ", "").Replace(SearchFields, "ID") + ")" + (inclBorken ? "" : "AND (FilesMissingIssues is null AND Is_Broken<>\"Yes\")");
        ...
        public static int FixWithSpotifyDetails(DataSet SongRecord, int i, int noOfRec, OleDbConnection cnb, ProgressBar pB_ReadDLCs, RichTextBox rtxt_StatisticsOnReadDLCs, string windw, DateTime timestamp, SQLite.SQLiteConnection cnc)
        {
            ...
            catch (Exception ex) { var tust = "Spotify Error ..." + ex.Message; timestamp = UpdateLog(timestamp, tust, false, c("dlcm_TempPath"), "", "", null, null); }
            return trackno;
        }
        ...
        private void btn_AssesIfDuplicate_Click(object sender, EventArgs e)
        {
            ...
            var IsCensored = databox.SelectedRows[0].Cells["Is_Censored"].Value.ToString();
            var HasAcoustic = databox.SelectedRows[0].Cells["Has_Acoustic"].Value.ToString();
            ...
            var Has_Slide = databox.SelectedRows[0].Cells["Has_Slide"].Value.ToString();
            ...
            if (norows >= 1)
                foreach (var file in SongRecord)
                {
                    ...
                    frm_Duplicates_Management frm1 = new frm_Duplicates_Management(file, info, author, tkversion, DD, Bass, Guitar, Combo,
                        ...
                        fsz, unpackedDir, Is_MultiTrack == "No" ? "" : Is_MultiTrack, MultiTrack_Version, File_Creation_Date, j.ToString() + "" + "", Platformm, IsLive == "No" ? "" : IsLive, LiveDetails, IsAcoustic, dupli_reason, databox.SelectedRows[0].Cells["Song_Lenght"].Value.ToString(), "-",
                        ...
                        , IsMetalCover == "No" ? "" : IsMetalCover, IsUkulele == "No" ? "" : IsUkulele, HasCapo == "No" ? "" : HasCapo, txt_Description.Text, A440TunningFrecv
                        , Has_Slide, txt_DuplicateOf.Text.ToInt32(), IsCensored == "No" ? "" : IsCensored, HasAcoustic == "No" ? "" : HasAcoustic, cnb, cnc);
                    frm1.ShowDialog();
                    ...
                    if (frm1.Alternate_No != Alternate_No && !(frm1.Alternate_No is null) && frm1.Is_Alternate == "Yes") txt_Alt_No.Value = int.Parse(frm1.Alternate_No);
                    ...
                    if (frm1.isCensored != "") chbx_A_IsCensored.Checked = frm1.isCensored == "Yes" ? true : false;
                    if (frm1.hasAcoustic != "") chbx_A_HasAcoustic.Checked = frm1.hasAcoustic == "Yes" ? true : false;
                    ...
                    if (frm1.hasSlide != "") chbx_A_IsSlide.Checked = frm1.hasSlide == "Yes" ? true : false;
                    ...
                    if (frm1.hasSlide != "") chbx_A_IsSlide.Checked = frm1.hasSlide == "Yes" ? true : false;
                    //if (frm1.has != "") chbx_A_IsKaraoke.Checked = frm1.isKaraoke == "Yes" ? true : false;
        ...
        private void btn_Package_Click(object sender, EventArgs e)
        {
            ...
            var pack = ""; var brkn = 0;
            if (chbx_Format.Text == "iOS") { MessageBox.Show("Not yet working. Retail songs(packing from DLCManager) can be eliminated just new ones packaged are \n\nWiP"); return; }
            pB_ReadDLCs.Value = 0;/*chbx_Format.Text == "PS4" ||*/
    ...
    "WHERE Type=\"Profile\" AND Profile_Name=\"" + c("dlcm_Configurations") + "\" and Comments like \"%dlcm_AdditionalManipul%\" AND DisplayGroup IN ('Pack', 'General') ORDER BY DisplayGroup DESC";
            ...
            var repacked_PS4Path = c("dlcm_TempPath") + "\\0_repacked\\PS4";
            ...
            CreateTempFolderStructure(Temp_Path_Import, old_Path_Import, broken_Path_Import, dupli_Path_Import, dlcpacks, pathDLC, repacked_Path
                , repacked_XBOXPath, repacked_PCPath, repacked_MACPath, repacked_PSPath, repacked_PS4Path, log_Path, AlbumCovers_PSPath
            ...
            var frmt = chbx_Format.Text.Replace("_US", "").Replace("_JP", "").Replace("_EU", "");
            ...
            if (norows > 0) CreatePackingGroup(cmd, cnb, cmb_Filter.Text, norows, cnc, frmt);
            ...
            if (ConfigRepository.Instance()["dlcm_AdditionalManipul123"] == "Yes")
            {
                ...
                if (GetMEtaBeforePacking(0, pB_ReadDLCs, rtxt_StatisticsOnReadDLCs, null, timestamp, AppWD, cmd, c("dlcm_AdditionalManipul54") == "Yes" ? true : false, Groupss
                    , c("dlcm_SearchFields"), txt_OldPath.Text)) f = true;
            ...
            //if (chbx_FN.CheckState==CheckState.Indeterminate) txt_OldPath.Text=

            var ordno = 0; i = 0; pB_ReadDLCs.Maximum = norows; pB_ReadDLCs.Value = 0;
            foreach (var filez in SongRecord)
            {
                ...
                args += c("dlcm_AdditionalManipul76") + ";" + "" + ";" + "MainDB" + ";" + "" + (ordno + i) + ";" + spotystatus + ";" + ybstatus + ";" + ftpstatus + ";" + "No"
                    + ";" + (chbx_Format.Text == "PS4" ? "PS4" : "") + ";" + (chbx_FN.CheckState == CheckState.Indeterminate ? CleanTitle(txt_OldPath.Text) : "");
        ...
        private async void btn_Debug_Click(object sender, EventArgs e)
        {
            ...
            //                    "oggPreviewPath = REPLACE(oggPreviewPath, '" + OLD_Path + "','" + NEW_Path + "')" +
            //                    ";";
            FixAudiofileInconsist(0, pB_ReadDLCs, rtxt_StatisticsOnReadDLCs, maindbpb, timestamp, AppWD, SearchCmd, false, true);
        }
        ...
        private void btn_AddInstrumental_Click(object sender, EventArgs e)
        {
            ...
            ApplyLyric(fn, databox.Rows[i].Cells["Folder_Name"].Value.ToString(), cnb, txt_Platform.Text.ToString().Replace("PC", "Pc")
                , txt_ID.Text, chbx_Lyrics.Checked, cnc, false);
            txt_Lyrics.Text =
        ...
        private void bth_ShiftVocalNotes_Click(object sender, EventArgs e)
        {
            ...
            if (cmb_Tracks.Text.ToString() != "") chbx_IsImprovedWithDM.Checked = true;
            ...
            , string txt_Platform, bool arrangoff, float not_p, string Song_Lenght, string txt_DuplicateOf, string Folder_Name, string txt_Title)
        {
            ...
            else
            {
                ...
                if (not_t.Split(';')[0] == "0") result11 = MessageBox.Show("You are trying to shift the Arrangements notes by 0 seconds (num_Lyrics is set to: " + num_Lyrics.ToString() + ")" +
            ...
            if (tsst == "")
                MessageBox.Show((cmb_Tracks is null ? "" : (cmb_Tracks.ToString() != "" ? "" : "(Single)")) + "Track " + lwnght + " shifted by " +
                    (not_t.Contains(";") ? (not_t.Split(';')[0] + "%" + ", starting time: " + not_t.Split(';')[1] + ", end time: " + not_t.Split(';')[2] + ", last actual note: " + not_t.Split(';')[5])
                    : num_Lyrics));
            else
                if (dus.Tables.Count > 0) MessageBox.Show("Track " + dus.Tables[0].Rows[0].ItemArray[1].ToString() + " was NOT shifted by " + (not_t == "0" ? num_Lyrics :
                    ...
                    + "\n\nDue to: " + tsst);
                else if (dus.Tables.Count > 0) MessageBox.Show("Issue at shifting notes!");
        ...
        public static async void PlayPreview(object sender, DoWorkEventArgs e)
        {
            ...
            {
                var timestamp = UpdateLog(DateTime.Now, "Play preview issue: " + ex.Message, true, ConfigRepository.Instance()["dlcm_TempPath"], "", "DLCManager", null, null);
            }
        ...
        public void Btn_ExportGuitarPro_Click(object sender, EventArgs e)
        {
            ...
            var txt = "Directory: " + dir + ", contains";
            foreach (DataRow myRow in dt.Tables[0].Rows)
            {
                ...
                for (var j = 0; j < GetNoRec(dz, cnb, cnc); j++)//dz.Tables[0].Rows.Count
                {
                    ...
                    //{
                    var fileName = dz.Tables[0].Rows[j].ItemArray[0].ToString();//idd+" "+myRow["Artist"].ToString() + " - " + myRow["Song_Title"].ToString(); 
                    ...
                    txt += myRow["Artist"].ToString() + " - " + myRow["Song_Title"].ToString() + " song)";
                }
            }
            MessageBox.Show(txt);
        }
        ...
        public string GenSearchGoTo(string cmd)
        {
            ...
            cm += (txt_Title.Text != "" ? " " + (chbx_ANDArtist.Checked ? "AND" : (chbx_ANDArtist.Checked ? "OR" : "AND")) + " Song_Title Like '%" + txt_Title.Text + "%'" : "");
            cm += (txt_Album.Text != "" ? " " + (chbx_ANDAlbum.Checked ? "OR" : (chbx_ANDAlbum.Checked ? "OR" : "AND")) + " Album Like '%" + txt_Album.Text + "%'" : "");
            ...
            cmd += (txt_OldPath.Text != "" ? " " + (chbx_ANDFileName.Checked ? "AND" : (chbx_ANDFileName.Checked ? "OR" : "AND")) + " Original_FileName Like '%" + txt_OldPath.Text + "%'" : "");
            cmd += (txt_Author.Text != "" ? " AND Author Like '%" + txt_Author.Text + "%'" : "");
            ...
            cmd = cmd.Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ");
            cmd = cmd.Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR");
            cmd = cmd.Replace("Main u  OR", "Main u WHERE").Replace("Main u WHERE", "Main u WHERE").Replace("Main u OR", "Main u WHERE").Replace("Main u OR", "Main u WHERE");
            cmd = cmd.Replace("WHERE  OR", "WHERE");
            cmd = cmd.Replace("WHERE  OR", "WHERE").Replace("WHERE  OR", "WHERE").Replace("WHERE OR", "WHERE").Replace("WHERE OR", "WHERE");
            cmd = cmd.Replace("WHERE OR", "WHERE ");
            cmd = cmd.Replace("OR ORDER BY ", "ORDER BY ");
            ...
            if (txt_Artist.Text is not null || txt_Title.Text is not null || txt_Album.Text is not null || txt_Description.Text != "" ||
                txt_Live_Details.Text != "" || txt_OldPath.Text != "" || txt_Author.Text != "" || txt_ID.Text != "")
            {
                ...
                cmdz = cmdz.Replace("WHERE AND", "WHERE ");
                cmdz = cmdz.Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR").Replace("OR OR", "OR");
                cmdz = cmdz.Replace("Main u  OR", "Main u WHERE").Replace("Main u WHERE", "Main u WHERE").Replace("Main u OR", "Main u WHERE").Replace("Main u OR", "Main u WHERE");
                cmdz = cmdz.Replace("WHERE  OR", "WHERE");
                cmdz = cmdz.Replace("WHERE  OR", "WHERE").Replace("WHERE  OR", "WHERE").Replace("WHERE OR", "WHERE").Replace("WHERE OR", "WHERE");
                cmdz = cmdz.Replace("WHERE OR", "WHERE ");
                cmdz = cmdz.Replace("OR ORDER BY ", "ORDER BY ");
                ...
                if (additionallyfound is not null)
                {
                    ...
                    "also due to additonal findings based on original, at import, metadata (Artist,title,Album, Author,Filename)",
        ...
        {
            var j = mouseLocation is not null ? mouseLocation.RowIndex : -1;
            if (j < 0) return;
        ...
        private void btn_GatherAttributes_Click(object sender, EventArgs e)
        {
            ...
            for (var h = 0; h < SongRecord[0].NoRec.ToInt32(); h++)
            {
                ...
                var IsInstrumental = SongRecord[h].Is_Instrumental; var IsEP = SongRecord[h].Is_EP; var IsUncensored = SongRecord[h].Is_Uncensored; var IsCensored = SongRecord[h].Is_Censored; var HasAcoustic = SongRecord[h].Has_Acoustic; var IsFullAlbum = SongRecord[h].Is_FullAlbum; var IsRemastered = SongRecord[h].Is_Remastered; var InTheWorks = SongRecord[h].IntheWorks;
                ...
                var ret = GetExtraAttributes(SongRecord[h].Original_FileName, SongDisplayName, SongDisplayName, SongDisplayName, SongRecord[h].Album, SongRecord[h].Author, SongRecord[h].DLC_Name, SongRecord[h].Artist, SongRecord[h].Song_Title);
                ...
                IsTVTheme = ag[26]; IsAmateurCover = ag[27]; IsMetalCover = ag[28]; IsUkulele = ag[29]; IsCensored = ag[30];
                if (pB_ReadDLCs != null) pB_ReadDLCs.Value += 1;
                //"(No.)", "[No.]"),Regex.Replace(
                ...
                if (SongRecord[h].Is_Censored != "Yes" && IsCensored == "Yes") { sel += " Is_Censored = \"" + IsCensored + "\", "; changed += "\nIs_Censored=\"" + SongRecord[h].Is_Censored + " -> " + IsCensored; }
                if ((SongRecord[h].Has_Acoustic != "Yes" && HasAcoustic == "Yes") || (SongRecord[h].Is_Acoustic != "Yes" && IsAcoustic == "Yes")) { sel += " Has_Acoustic = \"" + HasAcoustic + "\", "; changed += "\nHas_Acoustic=\"" + SongRecord[h].Has_Acoustic + " -> " + HasAcoustic; }
        ...
        private void btn_MKlink_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                CreateTempFolderStructure(v, v + "\\0_old", v + "\\0_broken", v + "\\0_duplicate", v + "\\0_dlcpacks", c("dlcm_RocksmithDLCPath"),
                       v + "\\0_repacked", v + "\\0_repacked\\XBOX360", v + "\\0_repacked\\PC", v + "\\0_repacked\\MAC", v + "\\0_repacked\\PS3", v + "\\0_repacked\\PS4"
            ...
            {
                var tsst = "Error at folder create ..." + ex.Message; timestamp = UpdateLog(timestamp, tsst, false, c("dlcm_TempPath"), "", "", null, null);
            }
        ...
        private void hasacousticToolStripMenuItem_Click(object sender, EventArgs e)
        {
            applyricghtclick(chbx_A_HasAcoustic, "Has_Acoustic", "Yes", "", "", "");
        }
        ...
        private void btn_Distribute_Evenly_Click(object sender, EventArgs e)
        {
            ...
            if (databox.SelectedCells.Count > 0)
            {
                ...
                EoFPath = databox.Rows[i].Cells["EoFPath"].Value.ToString();
            }
            ...
            {
                var timestamp = UpdateLog(DateTime.Now, "Distrib Evenly click: " + es.Message, true, ConfigRepository.Instance()["dlcm_TempPath"], "", "DLCManager", null, null);
            }
        }
        ...
        static long calc_FileSize(string s)
        {
            ...
            {
                var tsst = "Erro at file cal ..." + ex.Message; var timestamp = UpdateLog(DateTime.Now, tsst, false, c("dlcm_TempPath"), "", "", null, null);
            }
            return f;
        }
        ...
        private void btn_StemRolller_Click(object sender, EventArgs e)
        {
            ...
            var xx = "";
            if (File.Exists(paath)) xx = paath;
            else xx = Path.Combine(AppWD, "c:\\Programs\\StemRoller\\StemRoller.exe");
            if (!File.Exists(paath)) xx = "\\192.168.1.100\\Programs\\stemroller-2.1.1-win-cuda\\StemRoller.exe";
            ...
            if (File.Exists(xx)) //&& File.Exists(replace(c("dlcm_DBFolder")))
                using (var DDC = new Process())
                {
                    DDC.StartInfo = startInfo; DDC.Start(); DDC.WaitForExit(1000 * 1 * 1);
                }
        }

        private void btn_BasedOn_GP_Click(object sender, EventArgs e)
        {
            StartProcesss(txt_BasedOn_GP.Text, null);
        }
        ...
        //14.Only Vocal
        //15.(Only BackTrack)
        //16.(No Drums No Vocal)
        //17.(No Guitar No Vocal)
        //18.(No Bass No Vocal)
        //19.(No Guitar No Drums)
        //    if (wav.Contains("Only Bass")) v = "bass.ogg"; //1
        //    else if (wav.Contains("Only Drums")) v = "drums.ogg";//2
        //    else if (wav.Contains("(Only BackTrack)")) v = "instrumental.ogg";//3
        //    else if (wav.Contains("Only Guitars")) v = "otherbass.ogg";//4
        //    else if (wav.Contains("Only Guitar")) v = "other.ogg";//5
        //    else if (wav.Contains("Only Vocal")) v = "vocals.ogg";//6
        //    else if (wav.Contains("Only Lead")) v = "lead.ogg";//7
        //    else if (wav.Contains("Only Rhythm")) v = "rhythm.ogg";//8
        //    else if (wav.Contains("No Bass")) v = "otherdrumsvocals.ogg";//9
        //    else if (wav.Contains("No Drums")) v = "otherbassvocals.ogg";//10
        //    else if (wav.Contains("No Guitar")) v = "bassdrumsvocals.ogg";//11
        //    else if (wav.Contains("No Vocal")) v = "otherbassdrums.ogg";//12
        //    else if (wav.Contains("No Guitars")) v = "drumsvocals.ogg";//13
        //    else if (wav.Contains("No Lead")) v = "bassrhythmdrumsvocals.ogg";//14
        //    else if (wav.Contains("No Rhythm")) v = "bassleaddrumsvocals.ogg";//15
        //    else if (wav.Contains("(No Drums No Vocal)"))  v = "otherbass.ogg";//16
        //    else if (wav.Contains("(No Guitar No Vocal)")) v = "bassdrums.ogg";//17
        //    else if (wav.Contains("(No Bass No Vocal)")) v = "otherdrums.ogg";//18
        //    else if (wav.Contains("(No Guitar No Drums)")) v = "bassvocals.ogg";//19
        private void btn_AddAudioaMultitrack_Click(object sender, EventArgs e)
        {
            MessageBox.Show("The Multi Track imported looks for:\n-bass.wav,\n-drums.wav,\n-instrumental.wav,\n-other.wav (guitar?),\n-vocals.wav\n\n" +
                "?,or if Steamroller 6-track AI-trainned Model is used in Stemroller win/mac version for example is used, also for:,\n-guitar.wav,\n-piano.wav?\n\n, then uses SOX program to mix them into:" +
                "\n-otherdrumsvocals\n-bassdrums\n-bassdrums\n-bassdrumsvocals\n-drumsvocals\n-otherbassdrums\n-bassvocals\n-rhythm\n-lead\n-?guitar?\n-?piano?\n\n" +
                ", then covmnert them and move them into the song folder multitrack subfolder to be used at packing if Multitrack selected");
            var v = "";
            using (var fbd = new VistaFolderBrowserDialog())
            {
                if (fbd.ShowDialog() != DialogResult.OK)
                ...
                var bass = ""; var guitar = ""; var vocals = ""; var back = ""; var drums = ""; var otherbassvocals = false; var bassvocals = false; var drumsvocals = false;
                var otherdrumsvocals = false; var bassdrumsvocals = false; var otherbassdrums = false; var otherbass = false; var otherdrums = false; var bassdrums = false;
                var rhythm = false; var lead = false; var piano = ""; var quitar = ""; var quitars = false;
                if (audio.Length == 0)
                {
                    MessageBox.Show("Select a folder with more than 0 WAVs (fyi Stemroller 4-track generates bass.wav, drums.wav, instrumental.wav, other.wav, vocals.wav ?and 6-track Model also piano&guitar?)" +
                    " "); return;
                }
                ...
                foreach (var wav in audio)
                {
                    ...
                    v = "";
                    if (wav.Contains("\\piano.wav") || wav.Contains(" piano.wav")) //only piano
                        piano = wav;
                    if (wav.Contains("\\guitar.wav") || wav.Contains(" guitar.wav")) //no piano 
                        quitar = wav;
                    if (wav.Contains("\\bass.wav") || wav.Contains(" bass.wav")) //1only bass
                        bass = wav;
                    else if (wav.Contains("\\drums.wav") || wav.Contains(" drums.wav")) //2only drums
                        drums = wav;
                    else if (wav.Contains("\\instrumental.wav") || wav.Contains(" instrumental.wav")) //3only backtrack
                        back = wav;
                    else if (wav.Contains("\\other.wav") || wav.Contains(" other.wav")) //4onlyguitar (and others eg piano)
                        guitar = wav;
                    else if (wav.Contains("\\vocals.wav") || wav.Contains(" vocals.wav")) //5Only vocals
                        vocals = wav;
                    else if (wav.Contains("\\otherdrumsvocals.wav") || wav.Contains(" otherdrumsvocals.wav"))//6No Bass
                        otherdrumsvocals = true;
                    else if (wav.Contains("\\bassdrums.wav") || wav.Contains(" bassdrums.wav"))//7
                        bassdrums = true;
                    else if (wav.Contains("\\bassdrumsvocals.wav") || wav.Contains(" bassdrumsvocals.wav"))//8
                        bassdrumsvocals = true;
                    else if (wav.Contains("\\drumsvocals.wav") || wav.Contains(" drumsvocals.wav")) //9Noguitars
                        drumsvocals = true;
                    else if (wav.Contains("\\otherbassdrums.wav") || wav.Contains(" otherbassdrums.wav"))//10
                        otherbassdrums = true;
                    else if (wav.Contains("\\otherbass.wav") || wav.Contains(" otherbass.wav"))//11
                        otherbass = true;
                    else if (wav.Contains("\\otherdrums.wav") || wav.Contains(" otherdrums.wav"))//12
                        otherdrums = true;
                    else if (wav.Contains("\\otherbassvocals.wav") || wav.Contains(" otherbassvocals.wav"))//13
                        otherbassvocals = true;
                    else if (wav.Contains("\\bassvocals.wav") || wav.Contains(" bassvocals.wav"))//14
                        bassvocals = true;
                    else if (wav.Contains("\\rhythm.wav") || wav.Contains(" rhythm.wav"))//15 only rhythm
                        rhythm = true;
                    else if (wav.Contains("\\lead.wav") || wav.Contains(" lead.wav"))//16 only lead
                        lead = true;
                }
                var AppWDg = ExternalApps.TOOLKIT_ROOT;
                pB_ReadDLCs.Increment(1);
                if (!otherdrums)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + guitar + "\" \"" + drums + "\" \"" + Path.GetDirectoryName(drums) + "\\otherdrums.wav" + "\"";
                    var tst = "1/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!drumsvocals)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + drums + "\" \"" + vocals + "\" \"" + Path.GetDirectoryName(drums) + "\\drumsvocals.wav" + "\"";
                    var tst = "2/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!otherbass)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + guitar + "\" \"" + bass + "\" \"" + Path.GetDirectoryName(drums) + "\\otherbass.wav" + "\"";
                    var tst = "3/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!otherdrumsvocals)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + Path.GetDirectoryName(vocals) + "\\otherdrums.wav\" \"" + vocals + "\" \"" + Path.GetDirectoryName(vocals) + "\\otherdrumsvocals.wav" + "\"";
                    var tst = "4/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!otherbassvocals)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + Path.GetDirectoryName(vocals) + "\\otherbass.wav\" \"" + vocals + "\" \"" + Path.GetDirectoryName(vocals) + "\\otherbassvocals.wav" + "\"";
                    var tst = "5/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!bassdrums)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + bass + "\" \"" + drums + "\" \"" + Path.GetDirectoryName(drums) + "\\bassdrums.wav" + "\"";
                    var tst = "6/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!bassdrumsvocals)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + Path.GetDirectoryName(vocals) + "\\bassdrums.wav\" \"" + vocals + "\" \"" + Path.GetDirectoryName(vocals) + "\\bassdrumsvocals.wav" + "\"";
                    var tst = "7/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!otherbassdrums)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + Path.GetDirectoryName(vocals) + "\\otherdrums.wav\" \"" + bass + "\" \"" + Path.GetDirectoryName(bass) + "\\otherbassdrums.wav" + "\"";
                    var tst = "8/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!bassvocals)
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + bass + "\" \"" + vocals + "\" \"" + Path.GetDirectoryName(bass) + "\\bassvocals.wav" + "\"";
                    var tst = "9/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!rhythm && File.Exists(Path.GetDirectoryName(vocals) + "\\rhythm.wav\""))
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + Path.GetDirectoryName(vocals) + "\\bassdrumsvocals.wav\" \"" + Path.GetDirectoryName(vocals) + "\\rhythm.wav\"" + " \"" + Path.GetDirectoryName(bass) + "\\rhythmbassdrumsvocals.wav" + "\"";
                    var tst = "10/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!lead && File.Exists(Path.GetDirectoryName(vocals) + "\\lead.wav\""))
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + Path.GetDirectoryName(vocals) + "\\bassdrumsvocals.wav\" \"" + Path.GetDirectoryName(vocals) + "\\lead.wav\"" + " \"" + Path.GetDirectoryName(bass) + "\\leadbassdrumsvocals.wav" + "\"";
                    var tst = "11/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);
                if (!quitars && File.Exists(Path.GetDirectoryName(vocals) + "\\guitar.wav\""))
                {
                    var xx = Path.Combine(AppWDg, c("dlcm_SoX"));
                    if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install SoX audio utility comamnd line.", c("dlcm_SoX_www"), c("dlcm_SoX_www"), false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
                    var yy = " -m \"" + Path.GetDirectoryName(vocals) + "\\bassdrumsvocals.wav\" \"" + Path.GetDirectoryName(vocals) + "\\guitar.wav\"" + " \"" + Path.GetDirectoryName(bass) + "\\nopiano.wav" + "\"";
                    var tst = "12/13" + yy; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                    StartProcesss(@xx, yy);
                }
                ; pB_ReadDLCs.Increment(1);

                audio = Directory.GetFiles(temppath, "*.wav", System.IO.SearchOption.AllDirectories);
                var l = audio.Length;

                DialogResult result1 = DialogResult.No; //var v = "";
                v = ""; var z = ""; var aud = "";
                foreach (var wav in audio)
                {
                    if ((wav.Contains("\\bass.wav") || wav.Contains(" bass.wav")) && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//1
                        v += "Only Bass, ";
                    else if ((wav.Contains("\\drums.wav") || wav.Contains(" drums.wav")) && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//2
                        v += "Only Drums, ";
                    else if ((wav.Contains("\\instrumental.wav") || wav.Contains(" instrumental.wav")) && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem")))) //3same as novocal?
                        v += "(Only BackTrack), ";
                    else if ((wav.Contains("\\other.wav") || wav.Contains(" other.wav")) && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//4
                        v += "Only Guitars, ";
                    else if ((wav.Contains("\\vocals.wav") || wav.Contains(" vocals.wav")) && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//5
                        v += "Only Vocal, ";
                    else if (wav.Contains("otherdrumsvocals.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//6
                        v += "No Bass, ";
                    else if (wav.Contains("otherbassvocals.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//7
                        v += "No Drums, ";
                    else if (wav.Contains("bassdrumsvocals.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//8
                        v += "No Guitar, ";
                    else if (wav.Contains("otherbassdrums.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem")))) //9same as instrumental?
                        v += "No Vocal, ";
                    else if (wav.Contains("otherbass.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//10
                        v += "(No Drums No Vocal), ";
                    else if (wav.Contains("bassdrums.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//11
                        v += "(No Guitar No Vocal), ";
                    else if (wav.Contains("otherdrums.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//12
                        v += "(No Bass No Vocal), ";
                    else if (wav.Contains("drumsvocals.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//14
                        v += "No Guitars, ";
                    else if (wav.Contains("bassvocals.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//15
                        v += "(No Guitar No Drums), ";
                    else if (wav.Contains("lead.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//16
                        v += "Only Lead, ";
                    else if (wav.Contains("rhythm.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//17
                        v += "(Only Rhythm), ";
                    else if (wav.Contains("rhythmbassdrumsvocals.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//18
                        v += "No Lead, ";
                    else if (wav.Contains("leadbassdrumsvocals.wav") && (!File.Exists(wav.Replace(".wav", ".ogg")) || !File.Exists(wav.Replace(".wav", ".wem"))))//19
                        v += "(No Rhythm), ";
                    else if (!(File.Exists(wav.Replace(".wav", ".ogg")) || File.Exists(wav.Replace(".wav", ".wem"))))
                    {
                        z += Path.GetFileName(wav) + "\n";
                        aud = wav;
                    }
                }
                MessageBox.Show("track unidentified !!in: " + Path.GetDirectoryName(aud) + "\n\nplease add: bass.wav, drums.wav, instrumental.wav, other.wav, vocals.wav to filename, then re-import.\n\n " + z);
                ...
                foreach (var wav in audio)
                {
                    if ((wav.Contains("\\bass.wav") || wav.Contains(" bass.wav")) && (File.Exists(wav.Replace(".wav", ".ogg")) || File.Exists(wav.Replace(".wav", ".wem"))))//1
                        ...
                        b += "(No Guitar No Vocal), ";
                    else if (wav.Contains("otherdrums.wav") && (File.Exists(wav.Replace(".wav", ".ogg")) || File.Exists(wav.Replace(".wav", ".wem"))))//12
                        b += "(No Bass No Vocal), ";
                    else if (wav.Contains("drumsvocals.wav") && (File.Exists(wav.Replace(".wav", ".ogg")) || File.Exists(wav.Replace(".wav", ".wem"))))//14
                        b += "No Guitars, ";
                    else if (wav.Contains("bassvocals.wav") && (File.Exists(wav.Replace(".wav", ".ogg")) || File.Exists(wav.Replace(".wav", ".wem"))))//15
                        b += "(No Guitar No Drums), ";
                    else if (wav.Contains("lead.wav") && (File.Exists(wav.Replace(".wav", ".ogg")) || File.Exists(wav.Replace(".wav", ".wem"))))//16
                        b += "Only Lead, ";
                    else if (wav.Contains("rhythm.wav") && (File.Exists(wav.Replace(".wav", ".ogg")) || File.Exists(wav.Replace(".wav", ".wem"))))//17
                        b += "(Only Rhythm), ";
                ...
                if (v != "" || b != "")
                {
                    result1 = MessageBox.Show((v == "" ? "" : " Do you want to convert? existing wav-s to OGG/WEM \n\t" + v.Replace(", ", "\n\t") + "\n ")
                        + (b == "" ? "" : "and Also (Yes) Overrite: \n\t" + b.Replace(", ", "\n\t") + "\n") + "in: \n\t" + temppath + "\\multitracks\\"
                        + "\n(Cancel) or Stop the Import/Conversion", MESSAGEBOX_CAPTION, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);//txt_SongFolder.Text 
                    if (result1 == DialogResult.Cancel) return;
                }

                pB_ReadDLCs.Value = 0; int i = 0; var t = 0;
                z = "";
                foreach (var wav in audio)
                {
                    pB_ReadDLCs.Step = 1;
                    pB_ReadDLCs.Maximum = l; pB_ReadDLCs.Value = i;
                    v = ""; pB_ReadDLCs.Increment(1); i++;
                    if (wav.Contains("\\bass.wav") || wav.Contains(" bass.wav"))//1
                        v = "Only Bass";
                    else if (wav.Contains("\\drums.wav") || wav.Contains(" drums.wav"))//2
                        v = "Only Drums";
                    else if (wav.Contains("\\instrumental.wav") || wav.Contains(" instrumental.wav")) //3same as novocal?
                        v = "(Only BackTrack)";
                    else if (wav.Contains("\\other.wav") || wav.Contains(" other.wav"))//4
                        v = "Only Guitars";
                    else if (wav.Contains("\\vocals.wav") || wav.Contains(" vocals.wav"))//5
                        v = "Only Vocal";
                    else if (wav.Contains("otherdrumsvocals.wav"))//6
                        v = "No Bass";
                    else if (wav.Contains("otherbassvocals.wav"))//7
                        v = "No Drums";
                    else if (wav.Contains("bassdrumsvocals.wav"))//8
                        v = "No Guitar";
                    else if (wav.Contains("otherbassdrums.wav")) //9same as instrumental?
                        v = "No Vocal";
                    else if (wav.Contains("otherbass.wav"))//10
                        v = "(No Drums No Vocal)";
                    else if (wav.Contains("bassdrums.wav"))//11
                        v = "(No Guitar No Vocal)";
                    else if (wav.Contains("otherdrums.wav"))//12
                        v = "(No Bass No Vocal)";
                    else if (wav.Contains("drumsvocals.wav"))//14
                        v = "No Guitars";
                    else if (wav.Contains("bassvocals.wav"))//15
                        v = "(No Guitar No Drums)";
                    else if (wav.Contains("lead.wav"))//16
                        v = "Only Lead";
                    else if (wav.Contains("rhythm.wav"))//17
                        v = "(Only Rhythm)";
                    else if (wav.Contains("rhythmbassdrumsvocals.wav"))//18
                        v = "No Lead";
                    else if (wav.Contains("leadbassdrumsvocals.wav"))//19
                        v = "(No Rhythm)";
                    else z += Path.GetFileName(wav) + "\n";

                    if (v != "")
                    {
                        var tst = (12 + i) + "/" + (13 + l) + " Check audio: " + v; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);

                        var ogg = !checkaudioifreadable(txt_SongFolder.Text + "\\multitracks\\" + Path.GetFileName(wav.Replace(".wav", ".ogg")), AppWD, timestamp, 1);
                        var wem = !checkaudioifreadable(txt_SongFolder.Text + "\\multitracks\\" + Path.GetFileName(wav.Replace(".wav", ".wem")), AppWD, timestamp, 1);
                        if (!ogg || !wem || result1 == DialogResult.Yes)
                        {
                            try
                            {
                                DeleteFromDB("Groups", "DELETE * FROM Groups WHERE Type=\"AudioAlternative\" AND CDLC_ID=\"" + txt_ID.Text + "\" AND Comments =\"" + wav + "\"", cnb, cnc);
                                if (File.Exists(txt_SongFolder.Text + "\\multitracks\\" + Path.GetFileName(wav.Replace(".wav", ".ogg"))))
                                    File.Delete(txt_SongFolder.Text + "\\multitracks\\" + Path.GetFileName(wav.Replace(".wav", ".ogg")));
                                if (File.Exists(txt_SongFolder.Text + "\\multitracks\\" + Path.GetFileName(wav.Replace(".wav", ".wem"))))
                                    File.Delete(txt_SongFolder.Text + "\\multitracks\\" + Path.GetFileName(wav.Replace(".wav", ".wem")));
                            }
                            catch (Exception ex)
                            {
                                tst = "Issue at overrite" + ex.Message; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                            }
                        }
                        else if (result1 == DialogResult.No) { t++; continue; }

                        var insertcmdd = "CDLC_ID, Groupz, Type, Date_Added, Comments";
                        var insertvalues = "\"" + txt_ID.Text + "\", \"" + v + "\", \"AudioAlternative\", \"" + DateTime.Now.ToString("yyyyMMdd HHmmssfff") + "\",\"" + wav + "\"";
                        tst = (12 + i) + "/" + (13 + l) + " convert and copy"; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                        ...
                        if (!checkaudioifreadable(wav.Replace(".wav", ".wem"), AppWD, timestamp, 1))
                        {
                            try
                            {
                                ...
                                chbx_AI_TrackAvail.Checked = true;
                            }
                            catch (Exception ex)
                            {
                                tst = "Issue at convert and copy" + ex.Message; timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                            }
                        }
                        else
                        {
                            continue; timestamp = UpdateLog(timestamp, "Issue when cheecking created files: " + wav.Replace(".wav", ".wem"), true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                        }
                        t++;
                    }
                    else
                ...
                if (t == l) MessageBox.Show("bass, drums, instrumental, other, vocals and Combos-of-some copied");
                else MessageBox.Show("incomplete multitrack import,copy or conv, try again: " + t + "/" + l +
                    " copied" + (z == "" ? "" : z + " track(s) unidentified !!\n\nplease add: bass.wav, drums.wav, instrumental.wav, other.wav, vocals.wav to filename, then reimport."));

                if (t > 1) { chbx_AlternateAvail.Checked = true; }
                AddAlternativeAudioTracksAvailability(txt_ID.Text);
                //clean TEmplate folders as now uniquie to each conversion
                DeleteTempWwiseTemplateFolder(AppWD);
            }
        }
        ...
        private void btn_RebuildIndivFields_Click(object sender, EventArgs e)
        {
            ...
            if (chbx_ExcludenonBlanks.Checked) cmd +=
                (chbx_RebuildField.Text.Contains("A440TunningFrecv") && chbx_RebuildField.Text.Contains("(Main)")
                ? " where Is_Broken<>\"Yes\" AND (A440TunningFrecv =\"\" " + "OR A440TunningFrecv is null)" : "") +
                (chbx_RebuildField.Text.Contains("Has_Acoustic") && chbx_RebuildField.Text.Contains("(Main)")
                ? " where Is_Broken<>\"Yes\" AND (Has_Acoustic =\"\" " + "OR Has_Acoustic is null)" : "") +
                (chbx_RebuildField.Text.Contains("Volume") && chbx_RebuildField.Text.Contains("(Main)")
                ? " where Is_Broken<>\"Yes\" AND ((Volume =\"\" " + "OR Volume is null) OR (Preview_Volume =\"\" " + "OR Preview_Volume is null)" : "") +
                (chbx_RebuildField.Text.Contains("Has_Alternate_Track") && chbx_RebuildField.Text.Contains("(Main)")
                ? " where Is_Broken<>\"Yes\" AND (Has_Alternate_Track =\"\" " + "OR Has_Alternate_Track is null)" : "") +
                (chbx_RebuildField.Text.Contains("Has_Sections") && chbx_RebuildField.Text.Contains("(Main)")
                ? " where Is_Broken<>\"Yes\" AND (Has_Sections =\"\" " + "OR Has_Sections is null)" : "") +
                (chbx_RebuildField.Text.Contains("Has_Arrangements") && chbx_RebuildField.Text.Contains("(Main)")
                ? " where Is_Broken<>\"Yes\" AND (Has_Bass='No' and Has_Lead='No' and Has_Rhythm='No' and Has_Combo='No' aND Has_Vocals='No')" : "") +
            (chbx_RebuildField.Text.Contains("Bass_Picking") && chbx_RebuildField.Text.Contains("(Main)")
            ? " where Is_Broken<>\"Yes\" AND Has_Bass='Yes' AND (Bass_Picking =\"\" " + "OR Bass_Picking is null)" : "") +
            (chbx_RebuildField.Text.Contains("PitchShiftableEsOrDd") && chbx_RebuildField.Text.Contains("(Main)")
            ? " where Is_Broken<>\"Yes\" AND (PitchShiftableEsOrDd =\"\" " + "OR PitchShiftableEsOrDd is null)" : "");

            if (chbx_OnlyNo.Checked) cmd +=
    (chbx_RebuildField.Text.Contains("A440TunningFrecv") && chbx_RebuildField.Text.Contains("(Main)")
    ? " where Is_Broken<>\"Yes\" AND (A440TunningFrecv =\"No\")" : "") +
    (chbx_RebuildField.Text.Contains("Has_Acoustic") && chbx_RebuildField.Text.Contains("(Main)")
    ? " where Is_Broken<>\"Yes\" AND (Has_Acoustic =\"No\")" : "") +
    (chbx_RebuildField.Text.Contains("Volume") && chbx_RebuildField.Text.Contains("(Main)")
    ? " where Is_Broken<>\"Yes\" AND (Volume = \"0\" OR Preview_Volume = \"0\" OR Volume = \"0.0\" OR Preview_Volume = \"0.0\" OR Volume = \"-0\" OR Preview_Volume = \"-0\")" : "") +
    (chbx_RebuildField.Text.Contains("Has_Alternate_Track") && chbx_RebuildField.Text.Contains("(Main)")
    ? " where Is_Broken<>\"Yes\" AND (Has_Alternate_Track =\"No\")" : "") +
    (chbx_RebuildField.Text.Contains("Has_Sections") && chbx_RebuildField.Text.Contains("(Main)")
    ? " where Is_Broken<>\"Yes\" AND (Has_Sections =\"No\")" : "") +
    (chbx_RebuildField.Text.Contains("Has_Arrangements") && chbx_RebuildField.Text.Contains("(Main)")
    ? " where Is_Broken<>\"Yes\" AND (Has_Bass='No' and Has_Lead='No' and Has_Rhythm='No' and Has_Combo='No' aND Has_Vocals='No')" : "") +
(chbx_RebuildField.Text.Contains("Bass_Picking") && chbx_RebuildField.Text.Contains("(Main)")
? " where Is_Broken<>\"Yes\" AND Has_Bass='Yes' AND (Bass_Picking =\"No\")" : "") +
(chbx_RebuildField.Text.Contains("PitchShiftableEsOrDd") && chbx_RebuildField.Text.Contains("(Main)")
? " where Is_Broken<>\"Yes\" AND (PitchShiftableEsOrDd =\"\" " + "OR PitchShiftableEsOrDd is null)" : "");

            if (chbx_OnlyNo.Checked & chbx_ExcludenonBlanks.Checked) cmd = cmd.Replace(") where ", ") AND ");

            MainDBfields[] SongRecord = new MainDBfields[20000];
            SongRecord = GetRecord_s(cmd, cnb, cnc);
            pB_ReadDLCs.Maximum = SongRecord[0].NoRec.ToInt32();
            pB_ReadDLCs.Value = 0;
            var tst = "Records to pass through to rebuild some individual attributes" + SongRecord[0].NoRec.ToInt32(); timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            pB_ReadDLCs.Value = 0; pB_ReadDLCs.Step = 1;
            pB_ReadDLCs.Maximum = SongRecord[0].NoRec.ToInt32();
            tabControl.SelectedIndex = 0; var main = 0; var arrang = 0; var neg = 0;
            DataSet dgr = new DataSet(); arrang++;

            //Clear existing attributes
            if (chbx_RebuildField.Text.Contains("(Arrangements)") && !chbx_ExcludenonBlanks.Checked)
                dgr = UpdateDB("Arrangements", "UPDATE Arrangements SET " + chbx_RebuildField.Text.Substring(0, chbx_RebuildField.Text.IndexOf("(") - 1) + "=\"\"", cnb, cnc);
            if (chbx_RebuildField.Text.Contains("(Main)") && !chbx_RebuildField.Text.Contains("Has_Arrangements") && !chbx_ExcludenonBlanks.Checked && !chbx_OnlyNo.Checked)
                dgr = UpdateDB("Main", "UPDATE Main SET " + chbx_RebuildField.Text.Substring(0, chbx_RebuildField.Text.IndexOf("(") - 1) + "=\"\"", cnb, cnc);
            ...
            for (var h = 0; h < SongRecord[0].NoRec.ToInt32(); h++)
            {
                ...
                //check Arrangements
                var sect = "";
                var cmds = "SELECT * FROM Arrangements WHERE CDLC_ID=" + SongRecord[h].ID + GetArrOfficSQLTxt(arrangoff) +
                     (chbx_ExcludenonBlanks.Checked && chbx_RebuildField.Text.Contains("A440TunningFrecv") && chbx_RebuildField.Text.Contains("(Arrangements)")
                ? " where (A440TunningFrecv =\"\" " + "OR A440TunningFrecv is null)" : "") + ";";
                DataSet dss = new DataSet(); dss = SelectFromDB("Arrangements", cmds, "", cnb, cnc);
                ...
                for (var k = 0; k < noOfArr; k++)
                {
                    ...
                    if (chbx_RebuildField.Text.Contains("Arrangements"))
                    {
                        var t = GetArrangement(ms2);
                        if (t == "Lead" && !sel.Contains("Has_Lead")) sel += "Has_Lead=\"Yes\", ";
                        else
                            if (t == "Bass" && !sel.Contains("Has_Bass")) sel += "Has_Bass=\"Yes\", ";
                            else
                                if (t == "Combo" && !sel.Contains("Has_Combo")) sel += "Has_Combo=\"Yes\", ";
                                else
                                    if (t == "Rhythm" && !sel.Contains("Has_Rhythm")) sel += "Has_Rhythm=\"Yes\", ";
                                    else
                                        if ((t == "Vocals" || t == "JVocals") && !sel.Contains("Has_Vocals")) sel += "Has_Vocals=\"Yes\", ";
                    }
                    if (!sel.Contains("Bass_Picking") && chbx_RebuildField.Text.Contains("Bass_Picking"))
                    {
                        var t = GetBassPicking(ms2);
                        if (t != "") sel += "Bass_Picking=\"" + t + "\", ";
                    }
                    if (!sel.Contains("PitchShiftableEsOrDd") && chbx_RebuildField.Text.Contains("PitchShiftableEsOrDd"))
                    {
                        var t = GetPitchShiftableEsOrDd(ms2);
                        if (t != "") sel += "PitchShiftableEsOrDd=\"" + t + "\", ";
                    }
                    if (!sel.Contains("Has_Sections") && sect != "" && chbx_RebuildField.Text.Contains("Sections")) sel += "Has_Sections=\"Yes\", ";// break; }
                    if (!arr.Contains("Sections") && sect != "" && sect != "" && chbx_RebuildField.Text.Contains("Sections")) arr += "Sections=\"Yes\", ";
                    if (chbx_RebuildField.Text.Contains("Alternate") && chbx_RebuildField.Text.Contains("(Arrangements)")) if (GetHasAlternate(ms2)) arr += "Alternate=\"Yes\", ";//                    !sel.Contains("Alternate") && 
                    if (chbx_RebuildField.Text.Contains("Has_Alternate_Track") && chbx_RebuildField.Text.Contains("(Main)"))
                    {
                        if (GetHasAlternate(ms2)) { sel += "Has_Alternate_Track=\"Yes\", "; break; }//!sel.Contains("Has_Alternate_Track") && 
                        else break;
                    }

                    if (chbx_RebuildField.Text.Contains("Has_Acoustic") && chbx_RebuildField.Text.Contains("(Main)")) if (GetAcoustic(ms2)) { sel += "Has_Acoustic=\"Yes\", "; break; }
                    else break;//!sel.Contains("Has_Alternate_Track") && 

                    if (chbx_RebuildField.Text.Contains("Volume") && chbx_RebuildField.Text.Contains("(Main)"))
                    {
                        var v = GetVolumes(ms2); if (v is null) break;
                        sel += "Volume=\"" + v.Split(';')[0] + "\", Preview_Volume=\"" + v.Split(';')[1] + "\"";
                        break;
                    }//if (GetAcoustic(ms2)) !sel.Contains("Has_Alternate_Track") && 

                    if (chbx_RebuildField.Text.Contains("A440TunningFrecv") && chbx_RebuildField.Text.Contains("(Main)"))/*!sel.Contains("A440TunningFrecv") && */
                    {// sel += "A440TunningFrecv=\"" + GetTuningFrecv(ms2, false) + "\", ";
                        var t = GetTuningFrecv(ms2, false, SongRecord[h].ID);
                        if (t != "")
                            if (!sel.Contains(t) && t != "" && sel != "UPDATE Main SET ") { sel = "UPDATE Main SET " + "A440TunningFrecv=\"Different\", "; neg++; break; }
                            else sel = "UPDATE Main SET " + "A440TunningFrecv=\"" + t + "\", ";
                    }

                    if (chbx_RebuildField.Text.Contains("A440TunningFrecv") && chbx_RebuildField.Text.Contains("(Arrangements)"))//!arr.Contains("A440TunningFrecv") && 
                        arr += "A440TunningFrecv=\"" + GetTuningFrecv(ms2, false, SongRecord[h].ID) + "\", ";
                    ...
                    {
                        arr += " WHERE ID=" + ID;
                        DataSet dfr = new DataSet(); arrang++;
                        dfr = UpdateDB("Arrangements", arr.Replace(", WHERE", " WHERE").Replace(",  WHERE", " WHERE") + ";", cnb, cnc);
                ...
                {
                    sel += " WHERE ID=" + SongRecord[h].ID;
                    DataSet ddr = new DataSet(); main++;
                    ddr = UpdateDB("Main", sel.Replace(", WHERE", " WHERE").Replace(",  WHERE", " WHERE").Replace(",  WHERE ", " WHERE") + ";", cnb, cnc);
                    tst = SongRecord[h].ID + "/" + h + "/" + SongRecord[0].NoRec.ToInt32() + " - " + SongRecord[h].Artist + " - " + SongRecord[h].Song_Title;
                    timestamp = UpdateLog(timestamp, tst, true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
                }
            ...
            if (main > 0 || arrang > 0) MessageBox.Show(
                chbx_RebuildField.Text.Contains("(Main)") ? "Main updated " + main + "(" + neg + " as Different if Tunning Frecv)" +
                " for the " + chbx_RebuildField.Text + " flag" : "Arrangements updated " + arrang + " for the " + chbx_RebuildField.Text + " flag"
                + "\n\nFrom the original: " + SongRecord[0].NoRec.ToInt32() + "\n\n where there are still some: " + GetNoRecords(cmd, cnb, cnc) + " Empty");
        }
        ...
        private void btn_Export2ASCII_Click(object sender, EventArgs e)
        {
            var txt = "";
            ...
            //1 is string
            //2 is chord or notes
            string[,,] nt0 = new string[10, 3000, 5];//consolidation of all other timings(0), incl, is it cord or note(4)
            string[,,] nt1 = new string[10, 3000, 5];
            string[,,] nt2 = new string[10, 3000, 5];
            string[,,] nt3 = new string[10, 3000, 5];
            string[,,] nt4 = new string[10, 3000, 5];
            string[,,] nt5 = new string[10, 3000, 5];
            string[,,] nt6 = new string[10, 3000, 5];
            string[,] chords = new string[2000, 7];
            string[,] arg = new string[10, 7];
            float min = float.Parse("100");
            ...
            DataSet dbn = new DataSet(); dbn = SelectFromDB("Arrangements", "SELECT ArrangementType, XMLFilePath, JSONFilePath, RouteMask FROM Arrangements WHERE ArrangementType IN (\"Bass\",\"Guitar\") AND CDLC_ID=\"" + txt_ID.Text + "\"" + GetArrOfficSQLTxt(arrangoff), "", cnb, cnc);
            var norec = dbn.Tables.Count < 1 ? 0 : dbn.Tables[0].Rows.Count;
            var t = 1; float snglng = 0;
            if (norec > 0)
                for (int k = 0; k < norec; k++)
                {
                    ...
                    Platform platform = Folder_Name.GetPlatform();
                    var manifestFunctions = new ManifestFunctions(platform.version);
                    Song2014 xmlContent = null;
                    try
                    {
                        ...
                        {  //Save a copy
                            ...
                            if (!File.Exists(json + ".old8")) File.Copy(json, json + ".old8", false);
                            else File.Copy(json, json + ".old8", true);
                            xml = xml + ".old8";
                            json = json + ".old8";
                            RemoveDD(Folder_Name, chbx_Original.Checked ? "Yes" : "No", xml, platform, false, false, chbx_UseInternalDDRemovalLogic.Checked ? "Yes" : "No");
                            xmlContent = Song2014.LoadFromFile(xml);
                        ...
                        arg[k, 0] = dbn.Tables[0].Rows[k][0].ToString().ToLower();
                        arg[k, 1] = dbn.Tables[0].Rows[k][3].ToString().ToLower();
                        arg[k, 2] = xml;
                        for (var n = 0; n < xmlContent.ChordTemplates.Length; n++)
                        {
                            if (xmlContent.ChordTemplates[n].Fret0 > -1) chords[n, 1] = xmlContent.ChordTemplates[n].Fret0.ToString();
                        ...
                        if (xmlContent.Levels[0].Chords.Length > 0)
                            for (var m = 0; m < xmlContent.Levels[0].Chords.Length; m++)
                            {
                                if (chords[xmlContent.Levels[0].Chords[m].ChordId, 1] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt0[k, t, 3] = "c"; nt1[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt1[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 1]; }
                                if (chords[xmlContent.Levels[0].Chords[m].ChordId, 2] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt0[k, t, 3] = "c"; nt2[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt2[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 2]; }
                                if (chords[xmlContent.Levels[0].Chords[m].ChordId, 3] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt0[k, t, 3] = "c"; nt3[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt3[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 3]; }
                                if (chords[xmlContent.Levels[0].Chords[m].ChordId, 4] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt0[k, t, 3] = "c"; nt4[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt4[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 4]; }
                                if (chords[xmlContent.Levels[0].Chords[m].ChordId, 5] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt0[k, t, 3] = "c"; nt5[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt5[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 5]; }
                                if (chords[xmlContent.Levels[0].Chords[m].ChordId, 6] is not null) { nt0[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt0[k, t, 3] = "c"; nt6[k, t, 0] = xmlContent.Levels[0].Chords[m].Time.ToString(); nt6[k, t, 1] = chords[xmlContent.Levels[0].Chords[m].ChordId, 6]; }
                                t++;
                            }
                        if (xmlContent.Levels[0].Notes.Length > 0)
                            for (var n = 0; n < xmlContent.Levels[0].Notes.Length; n++)
                            {
                                if (xmlContent.Levels[0].Notes[n].String == 0) { nt0[k, n, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt0[k, t, 3] = "n"; nt1[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt1[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                else if (xmlContent.Levels[0].Notes[n].String == 1) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt0[k, t, 3] = "n"; nt2[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt2[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                else if (xmlContent.Levels[0].Notes[n].String == 2) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt0[k, t, 3] = "n"; nt3[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt3[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                else if (xmlContent.Levels[0].Notes[n].String == 3) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt0[k, t, 3] = "n"; nt4[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt4[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                else if (xmlContent.Levels[0].Notes[n].String == 4) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt0[k, t, 3] = "n"; nt5[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt5[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                else if (xmlContent.Levels[0].Notes[n].String == 5) { nt0[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt0[k, t, 3] = "n"; nt6[k, t, 0] = xmlContent.Levels[0].Notes[n].Time.ToString(); nt6[k, t, 1] = xmlContent.Levels[0].Notes[n].Fret.ToString(); }
                                t++;
                            }
                        ...
                        for (var n = 1; n <= xmlContent.Levels[0].Notes.Length + xmlContent.Levels[0].Chords.Length - 1; n++)
                            for (var m = n + 1; m <= xmlContent.Levels[0].Notes.Length + xmlContent.Levels[0].Chords.Length; m++)
                            {
                                ...
                                if (timesn is not null && timesm is not null)
                                {
                                    ...
                                    {
                                        var z0 = nt0[k, n, 0]; nt0[k, n, 0] = nt0[k, m, 0]; nt0[k, m, 0] = z0;
                                        var z1 = nt1[k, n, 1]; nt1[k, n, 1] = nt1[k, m, 1]; nt1[k, m, 1] = z1; var u1 = nt1[k, n, 0]; nt1[k, n, 0] = nt1[k, m, 0]; nt1[k, m, 0] = u1;
                                        var z2 = nt2[k, n, 1]; nt2[k, n, 1] = nt2[k, m, 1]; nt2[k, m, 1] = z2; var u2 = nt2[k, n, 0]; nt2[k, n, 0] = nt2[k, m, 0]; nt2[k, m, 0] = u2;
                                        var z3 = nt3[k, n, 1]; nt3[k, n, 1] = nt3[k, m, 1]; nt3[k, m, 1] = z3; var u3 = nt3[k, n, 0]; nt3[k, n, 0] = nt3[k, m, 0]; nt3[k, m, 0] = u3;
                                        var z4 = nt4[k, n, 1]; nt4[k, n, 1] = nt4[k, m, 1]; nt4[k, m, 1] = z4; var u4 = nt4[k, n, 0]; nt4[k, n, 0] = nt4[k, m, 0]; nt4[k, m, 0] = u4;
            ...
            for (int k = 0; k < norec; k++)
            {
                txt += "\n" + k + "/" + norec + " " + arg[k, 0].ToUpper() + " - " + arg[k, 1] + "\n";
                z = 0; done = false; var u = 10;
                do
                {
                    if (((z + min) / u) > 1)
                ...
                txt += parsenotes(nt1, min, t, snglng, k);
                txt += parsenotes(nt2, min, t, snglng, k);
                txt += parsenotes(nt3, min, t, snglng, k);
                txt += parsenotes(nt4, min, t, snglng, k);
                txt += parsenotes(nt5, min, t, snglng, k);
                txt += parsenotes(nt6, min, t, snglng, k);
                if (arg[k, 0].Contains(".old8")) File.Delete(arg[k, 1]);
            }

            ErrorWindow frm1 = new ErrorWindow(txt, "", "Quick Tab display of song-tracks & Notes", true, false, false, "OK", "", "", true);
            frm1.ShowDialog();
        }
        private string parsenotes(string[,,] trackarray, float min, int notesno, float songlenght, int track)
        {
            float z = 0; var done = false; var txt = "\n";
            for (int n = 1; n <= notesno; n++)
            {
                string time = trackarray[track, n, 0];
                string note = trackarray[track, n, 1];
                float nextn = 0;
                do
                {
                    if (time is not null)
                    {
                        nextn = float.Parse(time);
                        if (float.Parse(time) <= z + min) { txt += note; done = true; }
                        else txt += "-";
                    }
                    else
                    {
                        if (nextn == 0) for (int m = n + 1; m <= notesno; m++) { if (trackarray[track, m, 0] is not null) { nextn = float.Parse(trackarray[track, m, 0]); n = m - 1; break; } }
                        txt += "-";
                    }
                    if (z + min < nextn) z = z + min;/*!done+ min && */
                    else
                    {
                        done = true; z = z - min;
                    }
                }
                while (!done && z <= songlenght && z < nextn);
                done = false;
            }
            return txt;
        }
        ...
        private void button4_Click_3(object sender, EventArgs e)
        {
            using (var fbd = new OpenFileDialog())
            {
                ...
                {
                    var tard = "";
                    if (txt_EoFPath.Text == "") tard = Path.GetDirectoryName(txt_SongFolder.Text);
                    ...
                    txt_BasedOn_GP.Text += fbd.InitialDirectory + "\\" + filePath; //Path.Combine(Path.GetDirectoryName(info.Arrangements[0].SongXml.File), Path.GetFileName(fbd.SelectedPath));
                }
            }
        }

        private void btn_SetSpotifySongID_Click(object sender, EventArgs e)
        {
            Process.Start("IExplore.exe", txt_SpotifySongID.Text);
        }

        private void btn_OpenEoF_Click(object sender, EventArgs e)
        {
            StartProcesss(txt_EoFPath.Text, null);
        }
        ...
        private void txt_CustomsForge_Link_TextChanged(object sender, EventArgs e)
        {
            if (txt_CustomsForge_Link.Text != "")
            {
                btn_CustomForge_Link.Enabled = true;
                chbx_Found_on_CF.Checked = true;
            }
            else
            {
                btn_CustomForge_Link.Enabled = false;
                chbx_Found_on_CF.Checked = false;
            }
        }

        private void txt_SpotifySongID_TextChanged(object sender, EventArgs e)
        {
            if (txt_SpotifySongID.Text != "") btn_SetSpotifySongID.Enabled = true;
            else btn_SetSpotifySongID.Enabled = false;
        }

        private void txt_EoFPath_TextChanged(object sender, EventArgs e)
        {
            if (txt_EoFPath.Text != "") btn_OpenEoF.Enabled = true;
            else btn_OpenEoF.Enabled = false;
        }

        private void txt_BasedOn_Youtube_TextChanged(object sender, EventArgs e)
        {
            if (txt_BasedOn_Youtube.Text != "") btn_BasedOn_Youtube.Enabled = true;
            else btn_BasedOn_Youtube.Enabled = false;
        }

        private void txt_BasedOn_GP_TextChanged(object sender, EventArgs e)
        {
            if (txt_BasedOn_GP.Text != "") btn_BasedOn_GP.Enabled = true;
            else btn_BasedOn_GP.Enabled = false;
        }

        private void txt_BasedOn_CF_TextChanged(object sender, EventArgs e)
        {
            if (txt_BasedOn_CF.Text != "") btn_BasedOn_CF.Enabled = true;
            else btn_BasedOn_CF.Enabled = false;
        }

        private void txt_BasedOn_Tabs_TextChanged(object sender, EventArgs e)
        {
            if (txt_BasedOn_Tabs.Text != "") btn_BasedOn_Tabs.Enabled = true;
            else btn_BasedOn_Tabs.Enabled = false;
        }

        private void txt_filePathonself_TextChanged(object sender, EventArgs e)
        {
            if (txt_filePathonself.Text != "")
            {
                btn_filePathonself.Enabled = true;
                chbx_Found_on_CAPIRO.Checked = true;

            }
            else
            {
                btn_filePathonself.Enabled = false;
                chbx_Found_on_CAPIRO.Checked = false;
            }
        }

        private void txt_YouTube_Link_TextChanged(object sender, EventArgs e)
        {
            if (txt_YouTube_Link.Text != "") btn_Youtube.Enabled = true;
            else btn_Youtube.Enabled = false;
        }

        private void txt_Playthrough_TextChanged(object sender, EventArgs e)
        {
            if (txt_Playthrough.Text != "") btn_Playthrough.Enabled = true;
            else btn_Playthrough.Enabled = false;
        }

        private void chbx_A_IsInTheWorks_CheckedChanged(object sender, EventArgs e)
        {
            //chbx_A_IsInTheWorks.Checked = chbx_A_IsInTheWorks.Checked ? false : true;
        }

        private void chbx_A_IsTVTheme_CheckedChanged_1(object sender, EventArgs e)
        {

        }

        private void chbx_A_IsSoundtrack_CheckedChanged_1(object sender, EventArgs e)
        {

        }

        private void chbx_A_CatchAllAtributte_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (chbx_A_CatchAllAtributte.Text == "")
                return;
            //Update Song / Main DB
            var connection = new OleDbConnection("Provider=Microsoft." + c("dlcm_AccessDLLVersion") + ";Data Source=" + c("dlcm_DBFolder"));
            var command = connection.CreateCommand();
            command.CommandText = "UPDATE Main SET ";
            command.CommandText += chbx_A_CatchAllAtributte.Text + " = @param1 ";
            //command.CommandText += "Has_Alternate_Audio = @param146 ";
            command.CommandText += " WHERE ID = " + txt_ID.Text;

            command.Parameters.AddWithValue("@param1", "Yes");

            UpdateDBbyExecuteNonQuery(command, connection, cnc);
            System.Threading.Thread.Sleep(2000);
            MainDBfields[] SongRecord = new MainDBfields[20000];
            SongRecord = GetRecord_s("SELECT * FROM Main WHERE ID=" + txt_ID.Text, cnb, cnc);
            SetAttributes(SongRecord);
        }

        //how many instruments (lead rhythm, tunnings)
        //pick up sections
        private void btn_Process_ASCII_Click(object sender, EventArgs e)
        {
            Format_ASCII();
        }

        private void chbx_A_IsSlide_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void chbx_A_IsAmateurCover_CheckedChanged_1(object sender, EventArgs e)
        {

        }

        private void chbx_A_IsMetalCover_CheckedChanged_1(object sender, EventArgs e)
        {

        }

        private void btn_Gen_Orig_Audio_Click(object sender, EventArgs e)
        {
            var tgst = "Extract original audio in case it has been modified by the DLCManager tool e.g. reducinf the compression bitrate"; UpdateLog(DateTime.Now, tgst, false, c("dlcm_TempPath"), null, "MainDB", null, null);
            RocksmithToolkitLib.Platform SourcePlatform = (c("dlcm_TempPath") + "\\0_old\\" + txt_OldPath.Text).GetPlatform();
            var unpackedDir = Packer.Unpack(c("dlcm_TempPath") + "\\0_old\\" + Path.GetFileName(txt_OldPath.Text), c("dlcm_TempPath") + "\\0_temp\\", SourcePlatform, true, true);
            StartProcesss(c("dlcm_TempPath") + "\\0_temp\\", null);
            MessageBox.Show("Audio files in the " + txt_OldPath.Text + " extracted folder inside: " + c("dlcm_TempPath") + "\\0_temp\\");
        }

        private void chbx_AltTrack_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void chbx_A440_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void btn_RemoveMultiTracks_Click(object sender, EventArgs e)
        {
            var t = "SELECT * FROM Groups WHERE Type =\"AudioAlternative\" AND CDLC_ID=\"" + txt_ID.Text + "\"";
            DataSet dss = new DataSet(); dss = SelectFromDB("Groups", t, "", cnb, cnc);
            var a = GetNoRec(dss, cnb, cnc);
            DeleteFromDB("Groups", "DELETE * FROM Groups WHERE Type=\"AudioAlternative\" AND CDLC_ID=\"" + txt_ID.Text + "\"", cnb, cnc);
            DeleteDirectory(txt_SongFolder.Text + "\\multitracks", false);
            chbx_MultiTrack.Checked = false;
            MessageBox.Show(a + " MultiTrack-Groups and \"" + txt_SongFolder.Text + "\\multitracks" + "\" deleted (" + !Directory.Exists(txt_SongFolder.Text + "\\multitracks") + ")");
        }

        private void chbx_A_IsMultiStrings_CheckedChanged_1(object sender, EventArgs e)
        {

        }

        private void btn_CleanAllBroken_Click(object sender, EventArgs e)
        {
            var cnn = new OleDbConnection("Provider=Microsoft." + c("dlcm_AccessDLLVersion") + ";Data Source=" + c("dlcm_DBFolder"));
            var command = cnn.CreateCommand();
            command.CommandText = "UPDATE Main SET ";
            command.CommandText += "Is_Broken = @param8, ";
            command.CommandText += "FilesMissingIssues = @param9 ";
            command.CommandText += " WHERE ID IN (" + SearchCmd.Replace("*", "ID").Replace(";", "").Replace(SearchFields, "ID") + ")";
            ...
            databox.Visible = false; databox.Refresh(); databox.Visible = true;
            DataSet dhs = new DataSet(); dhs = SelectFromDB("Main", "Select * FROM Main WHERE ID IN(" + SearchCmd.Replace(" * ", "ID").Replace("; ", "").Replace(SearchFields, "ID") + ")", "", cnb, cnc);
            MessageBox.Show("All Filtered songs(" + GetNoRec(dhs, cnb, cnc) + ") in DB have had their Broken status removed and their FilesMissingIssues field cleaned");
        }

        private void btn_CheckOld_Click(object sender, EventArgs e)
        {
            var cmd = "SELECT * FROM Main WHERE"
                    + " ID IN(" + SearchCmd.Replace(" * ", " ID ").Replace("; ", "").Replace(SearchFields, "ID") + ")";
            DataSet dms = new DataSet(); dms = SelectFromDB("Main", cmd, "MainDB", cnb, cnc);
            var noOfRec = GetNoRec(dms, cnb, cnc);
            timestamp = UpdateLog(timestamp, noOfRec + "DLCs 4 checks of file existence and if more than zero kb", true, c("dlcm_TempPath"), "", "MainDB", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);

            if (pB_ReadDLCs != null) { pB_ReadDLCs.Value = 0; pB_ReadDLCs.Step = 1; pB_ReadDLCs.Maximum = noOfRec; }
            timestamp = UpdateLog(timestamp, "\nPath checked : "
                                + c("dlcm_TempPath") + "\\0_old\\", true, null, "", "DLCManager", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);
            var noold = 0; var nonoold = 0; var oldmissing = ""; var ids = "0,";
            for (var j = 0; j <= noOfRec - 1; j++)
            {
                if (pB_ReadDLCs != null) pB_ReadDLCs.Value += 1;

                var ID = dms.Tables[0].Rows[j].ItemArray[0].ToString();
                var OrigFileName = dms.Tables[0].Rows[j].ItemArray[19].ToString();
                var old = c("dlcm_TempPath") + "\\0_old\\" + OrigFileName;
                var hasOld = dms.Tables[0].Rows[j].ItemArray[87].ToString();

                timestamp = UpdateLog(timestamp, "\nOLD Check " + j + "/" + (noOfRec - 1) + " song: "
                    + old, true, null, "", "DLCManager", pB_ReadDLCs, rtxt_StatisticsOnReadDLCs);

                if (hasOld == "Yes")
                    if (!FileExistsAndIsNotZeroKB(old))
                    {
                        noold++;
                        ids += ID + ",";
                        oldmissing += "\n" + ID + ": " + old;
                    }
                if (hasOld != "Yes") { nonoold++; oldmissing += "\nNever Imported " + ID + ": " + old; }
            }
            DataSet dus = new DataSet(); cmd = "UPDATE Main SET Available_Old=\"No\", FilesMissingIssues=(REPLACE(FilesMissingIssues,\"Missin OLD\",\"\",1,-1,1)&\"Missin OLD\") WHERE ID IN (" + ids + "0);";
            if (ids != "0,")
            {
                dus = UpdateDB("Main", cmd, cnb, cnc);
                cmb_Filter.Text = "No Old (flag)";
            }
            ErrorWindow frm1 = new ErrorWindow("DLCs with original imported 'psarc's file saved but with the file missing or file-size zero:" + noold + "\n(now with Has_Old flag set to No)"
                 + "\n\nNo OLD/ORIG imported 'psarc's: " + nonoold + "\n\nList of files: " + oldmissing,
     "", "Check OLD files:", false, false, true, "", "", "OK", false);
            frm1.ShowDialog();

        }

        private void ToolStripMenuItemCensored_Click(object sender, EventArgs e)
        {
            applyricghtclick(chbx_A_IsCensored, "Is_Censored", "Yes", "", "", "");
        }

        private void chbx_FN_CheckStateChanged(object sender, EventArgs e)
        {
            int i = databox.SelectedCells.Count > 0 ? databox.SelectedCells[0].RowIndex : 0;
            if (chbx_FN.CheckState == CheckState.Checked)
            {
                MainDBfields[] SongRecord = new MainDBfields[20000];
                SongRecord = GetRecord_s("SELECT * FROM Main WHERE ID=" + databox.Rows[i].Cells["ID"].Value.ToString(), cnb, cnc);
        ...
        private void button1_Click_4(object sender, EventArgs e)
        {
            ...
            ErrorWindow frm1 = new ErrorWindow(txt_OldPath.Text,
"", "FileName to save under:", false, false, true, "", "", "OK (incl. Editable File Name)", false);
            frm1.ShowDialog();
            if (frm1.erorrfeedback != "")
            {
                txt_OldPath.Text = frm1.erorrfeedback + "[indicative at packing]";
                chbx_FN.CheckState = CheckState.Indeterminate;
            }
        }

        private void chbx_Clean_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void chbx_PackBeta_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void chbx_Original_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void btn_TuxGuitar_Click(object sender, EventArgs e)
        {
            var paath = c("dlcm_TuxGuitar");
            var xx = "";
            if (File.Exists(paath)) xx = paath;
            else xx = Path.Combine(AppWD, "c:\\Programs\\TuxGuitar\\TuxGuitar.exe");
            //if (File.Exists(paath)) xx = "\\192.168.1.100\\Programs\\TuxGuitar\\tuxguitar.bat";
            if (!File.Exists(xx)) { ErrorWindow frm1 = new ErrorWindow("Install TuxGuitar if you want to use it.", c("dlcm_TuxGuitar_www"), "Missing TuxGuitar!", false, false, true, "", "", "", false); frm1.ShowDialog(); return; }
        ...
        private void chbx_A_IsInTheWorks_CheckedChanged_1(object sender, EventArgs e)
        {

        }

        private void chbx_ANDArtist_CheckedChanged(object sender, EventArgs e)
        {
            chbx_ORArtist.Checked = chbx_ANDArtist.Checked ? false : true;
        }

        private void chbx_ORArtist_CheckedChanged(object sender, EventArgs e)
        {
            chbx_ANDArtist.Checked = chbx_ORArtist.Checked ? false : true;
        }

        private void chbx_ORAlbum_CheckedChanged(object sender, EventArgs e)
        {
            chbx_ANDAlbum.Checked = chbx_ORAlbum.Checked ? false : true;
        }

        private void chbx_ANDAlbum_CheckedChanged(object sender, EventArgs e)
        {
            chbx_ORAlbum.Checked = chbx_ANDAlbum.Checked ? false : true;
        }

        private void chbx_ANDFileName_CheckedChanged(object sender, EventArgs e)
        {
            chbx_ORFileName.Checked = chbx_ANDFileName.Checked ? false : true;
        }

        private void chbx_ORFileName_CheckedChanged(object sender, EventArgs e)
        {
            chbx_ANDFileName.Checked = chbx_ORFileName.Checked ? false : true;
        }

        private void btn_Export2Tab_Click(object sender, EventArgs e)
        {
            Btn_ExportGuitarPro_Click(null, null);
        }

        private void btn_AddTabbingInfo_Click(object sender, EventArgs e)
        {
            var packageData = DLCPackageData.LoadFromFolder(txt_SongFolder.Text, TargetPlatform, TargetPlatform);
            ConfigRepository.Instance()["dlcm_GlobalTempVariable"] = CleanInvalidXmlChars(
                txt_Author.Text + ";" + txt_APP_ID.Text + ";" + txt_Track_No.Text + ";" + //0-2
                txt_Version.Text + ";" + txt_ID.Text + ";" + txt_EoFPath.Text + ";" + //3-5
                txt_YouTube_Link.Text.Replace(";", ",") + ";" + txt_BasedOn_Youtube.Text.Replace(";", ",") + ";" + txt_BasedOn_CF.Text.Replace(";", ",") + ";" + //6-8
                txt_BasedOn_Tabs.Text.Replace(";", ",") + ";" + txt_SpotifySongID.Text.Replace(";", ",") + ";" + txt_Description.Text.Replace(";", ",") + ";" +//9-11
                 txt_ToDo_s.Text.Replace(";", ",") + ";" + txt_ToneDetails.Text.Replace(";", ",") + ";" + "Yes" + ";" +//12-14
                 "Yes" + ";" + "No" + ";" + ConfigRepository.Instance()["dlcm_EoFPath"] + ";" + //15-17
                txt_PackingDate.Text + ";" + txt_UpdateVersionDate.Text + ";" + txt_BasedOn_GP.Text + ";" +//18-20
               packageData.SongInfo.SongDisplayName + ";" + packageData.SongInfo.Artist + ";" + packageData.SongInfo.Album + ";" +//21-23                                                                                                           //  
                txt_PackageDetails.Text + ";" + txt_BasedOn_RB.Text + ";" + (chbx_A_IsSlide.Checked ? "Yes" : "No") + ";" + txt_CF_Author.Text + ";" +
                (chbx_A_HasAcoustic.Checked ? "Yes" : "No") + ";" + (chbx_A_ProductionReady.Checked ? "Yes" : "No") + ";"
            ...
            packageData.ToolkitInfo.PackageComment = ConfigRepository.Instance()["dlcm_GlobalTempVariable"];
            PackNew frm = new PackNew(packageData, cnb, cnc); frm.ShowDialog();
            ...
            var chbx_SaveInDBTxet = false; var chbx_SaveRemotelyText = false; var txt_PackageDate = ""; var txt_UpdateDate = ""; var txt_BasedOnGP = "";
            var SaveRemotelyPath = ""; /*var txt_Has_Slide = ""; var txt_CF_Authors = "";*/
            var versionffile = "";// Directory.Exists(unpackedDir) ? Directory.GetFiles(unpackedDir, "toolkit.version", System.IO.SearchOption.AllDirectories) : [""];
            var PackageComment = ""; var txt_Songtitle = ""; var txt_Artist = ""; var txt_Album = ""; var txt_PackageDetai = ""; var txt_BasedOnRB = "";
            var chbx_Has_Slide = ""; var txt_CFAuthor = ""; var chbx_HasAcoustic = ""; var chbx_ProductionReady = "";
            if (ConfigRepository.Instance()["dlcm_GlobalTempVariable"].Length > 0)            //    if (PackageComment != "") ;/*info.ToolkitInfo.PackageComment*/
                if (c("dlcm_GlobalTempVariable").IndexOf(";") > 0)
                    try
                    {
                        string[] agg = c("dlcm_GlobalTempVariable").Split(';');// info.ToolkitInfo.PackageComment.ToString().Split(';');
                        if (agg[15] == "Yes")
                        {
                            ...
                            txt_PackageDate = agg[18]; txt_UpdateDate = agg[19]; txt_BasedOnGP = agg[20];
                            txt_Songtitle = agg[21]; txt_Artist = agg[22]; txt_Album = agg[23]; txt_PackageDetai = agg[24]; txt_BasedOnRB = agg[25];
                            chbx_Has_Slide = agg[26]; txt_CFAuthor = agg[27]; chbx_HasAcoustic = agg[28]; chbx_ProductionReady = agg[29];
                            //if (info.SongInfo.Artist != txt_Artist) MessageBox.Show("diff artist than in Package Comment" + info.SongInfo.Artist + " - " + txt_Artist);
                            //if (info.SongInfo.Album != txt_Album) MessageBox.Show("diff Album than in Package Comment" + info.SongInfo.Album + " - " + txt_Album);
                            //if (info.SongInfo.SongDisplayName != txt_Songtitle) MessageBox.Show("diff Song title than in Package Comment" + info.SongInfo.SongDisplayName + " - " + txt_Songtitle);
                            if (txt_AuthorText is not null && txt_AuthorText != "" && txt_Author.Text != txt_AuthorText) txt_Author.Text = txt_AuthorText;
                            if (txt_TrackNoText is not null && txt_TrackNoText != "" && txt_Track_No.Text != txt_TrackNoText) txt_Track_No.Text = txt_TrackNoText;
                            if (txt_VersionText is not null && txt_VersionText != "" && txt_Version.Text != txt_VersionText) txt_Version.Text = txt_VersionText;
                            if (txt_EoFPathText is not null && txt_EoFPathText != "" && txt_EoFPath.Text != txt_EoFPathText) txt_EoFPath.Text = txt_EoFPathText;
                            if (txt_BasedOnYBText is not null && txt_BasedOnYBText != "" && txt_BasedOn_Youtube.Text != txt_BasedOnYBText) txt_BasedOn_Youtube.Text = txt_BasedOnYBText;
                            if (txt_YBLinkText is not null && txt_YBLinkText != "" && txt_YouTube_Link.Text != txt_YBLinkText) txt_YouTube_Link.Text = txt_YBLinkText;
                            if (txt_SpotifyText is not null && txt_SpotifyText != "" && txt_SpotifySongID.Text != txt_SpotifyText) txt_SpotifySongID.Text = txt_SpotifyText;
                            if (txt_DescriptionText is not null && txt_DescriptionText != "" && txt_Description.Text != txt_DescriptionText) txt_Description.Text = txt_DescriptionText;
                            if (txt_toDosText is not null && txt_toDosText != "" && txt_ToDo_s.Text != txt_toDosText) txt_ToDo_s.Text = txt_toDosText;
                            if (txt_ToneDetailsText is not null && txt_ToneDetailsText != "" && txt_ToneDetails.Text != txt_ToneDetailsText) txt_ToneDetails.Text = txt_ToneDetailsText;
                            if (txt_TabLinksText is not null && txt_TabLinksText != "" && txt_BasedOn_Tabs.Text != txt_TabLinksText) txt_BasedOn_Tabs.Text = txt_TabLinksText;
                            if (txt_BasedOnGP is not null && txt_BasedOnGP != "" && txt_BasedOn_GP.Text != txt_BasedOnGP) txt_BasedOn_GP.Text = txt_BasedOnGP;
                            if (txt_BasedOnCFText is not null && txt_BasedOnCFText != "" && txt_BasedOn_CF.Text != txt_BasedOnCFText) txt_BasedOn_CF.Text = txt_BasedOnCFText;
                            if (txt_BasedOnRB is not null && txt_BasedOnRB != "" && txt_BasedOn_RB.Text != txt_BasedOnRB) txt_BasedOn_RB.Text = txt_BasedOnRB;
                            if (txt_CFAuthor is not null && txt_CFAuthor != "" && txt_CF_Author.Text != txt_CFAuthor) txt_CF_Author.Text = txt_CFAuthor;
                            if (txt_PackageDetai is not null && txt_PackageDetai != "" && txt_PackageDetails.Text != txt_PackageDetai) txt_BasedOn_RB.Text = txt_PackageDetai;
                            if (txt_CFAuthor is not null && txt_CFAuthor != "" && txt_CF_Author.Text != txt_CFAuthor) txt_CF_Author.Text = txt_CFAuthor;
                            if (chbx_Has_Slide == "Yes" && !chbx_A_IsSlide.Checked) chbx_A_IsSlide.Checked = true;
                            if (chbx_HasAcoustic == "Yes" && !chbx_A_HasAcoustic.Checked) chbx_A_HasAcoustic.Checked = true;
                            if (chbx_ProductionReady == "Yes" && !chbx_A_ProductionReady.Checked) chbx_A_ProductionReady.Checked = true;
                        }
                    }
                    catch (Exception ef)
                    {
                        var tsst = "old metadata ..." + ef; timestamp = UpdateLog(timestamp, tsst, false, ConfigRepository.Instance()["dlcm_TempPath"], "", "", null, null);
                    }
            //PackageComment = ReadPackageComment(ConfigRepository.Instance()["dlcm_GlobalTempVariable"]);
        }
    }
}